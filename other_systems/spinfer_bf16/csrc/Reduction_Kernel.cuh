...
__global__ void SplitK_Reduction(__nv_bfloat16* C, __nv_bfloat16* Reduction_Workspace, int M_Global, int N_Global, int Split_K, bool Accumulate)
{
    // return;
...
#pragma unroll
    for (int j = 0; j < HALF_PER_128B; j++)
        Results[j] = Accumulate
            ? __bfloat162float(C_BasePTR_ThisBlock[threadIdx.x * HALF_PER_128B + j])
            : 0.0f;
    //
...
#pragma unroll
    for (int j = 0; j < HALF_PER_128B; j++)
        C_BasePTR_ThisBlock[threadIdx.x * HALF_PER_128B + j] = __float2bfloat16_rn(Results[j]);
}
