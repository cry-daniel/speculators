...
Keep these outputs in `examples/evaluate/eval-guidellm/results.bak/`. The
current fixed-block row-routed MLP path already routes important rows to dense
and unimportant rows to 2:4 sparse without dense overwrite. Treat the current
bottleneck as mixed-branch launch/shape overhead, underfilled sparse Tensor
Core work, CUDA Graph/cache hygiene, and request-drain effects until a fresh
seven-part report shows otherwise.

Latest explicit slowdown report:
...
Latest lossy systems note:

```text
SR24_LOSSY_PPOPP_OPTIMIZATION_PLAN_20260630.md
SR24_GROUPED_QUEUE_NEXT_DESIGN_20260702.md
...
Current 2026-06-30 lossy read: use an absolute `8 percentage point` accuracy
budget and GSM8K `limit >= 50` for quick gates. The current row-routed MLP
implementation already makes important rows dense-only and unimportant rows
2:4-only, so it avoids dense correction for sparse-only rows. It passes the
GSM8K-50 quality gate at `-2pp`, but still does not reach the `1.2x` target
across bs8/16/32/64. The remaining bottleneck is the mixed MLP operator and data
format: base-only 2:4 has a real speed upper bound, but sparse-base plus
dense-important routing currently erases the gain. Future SR24 work should
prioritize a fixed-capacity packed verifier-block route descriptor and a
fused/grouped mixed MLP kernel. Do not return to lossless gates or scalar
threshold sweeps unless the operator path changes.

2026-07-02 base-only diagnosis refresh:
`examples/evaluate/eval-guidellm/scripts/analyze_sr24_baseonly_slowdown.py`
now emits an aggregate accepted-length/GPU-util/CUDA-Graph read. The current
refresh is:
...
It analyzes 8 `base_only_24` rows. Six are slower than dense, but all six are
`not_acceptance_limited`: accepted draft tokens per step are higher than dense.
Those slow rows are instead GPU-underutilized, graph-lost, or overbroad-scope
rows. Two graph-enabled MLP-only rows are faster than dense with healthy
acceptance/GPU utilization (`1.697x` and `1.844x` full-batch speedup). Treat
`base_only_24` as evidence that 2:4 sparse MLP has compute headroom when graph
and shape are healthy; do not diagnose it as an accepted-length collapse.

2026-07-02 all-corrected backend refresh:
`examples/evaluate/eval-guidellm/scripts/analyze_sr24_allcorrected_path.py`
summarizes exact `all_corrected_24` backend/device evidence from existing
result roots. Current output:
...
It analyzes 5 `all_corrected_24` rows, 4 with serving throughput. Three
`compressed_dense` rows are proven GPU-resident
(`residual_device_counts={"cuda:0":...}`,
`compressed_residual_runtime_on_gpu=True`, no non-GPU modules). All 4 measured
rows are slower than dense while accepted draft length is intact. Therefore the
current exact all-corrected path is not slow because `compressed_dense` falls
back to CPU or because acceptance collapses; it is slow because it pays
two-pass sparse-base plus residual-correction work. Keep all-corrected as a
dense-equivalent correctness/control path until there is a fused/packed exact
operator.

2026-07-02 near-full fixed-capacity update: the current best live SR24 knob is
`lossy_prefix2_rowrouted_mlp` with `SPECLINK_SR24_SELECTIVE_MIN_PREFIX_RESIDUAL=2`,
front28 no-verify dense protection, `SPECLINK_SR24_SCHEDULER_POLICY_NEAR_FULL_TOLERANCE=2`,
and `SPECLINK_SR24_FIXED_BLOCK_CAPACITY_PADDING=1`. This pads active verifier
blocks at 62/63 requests up to the bs64 policy capacity with dummy rows, runs
dense-important and 2:4-sparse-unimportant branches, and assembles only real
rows. It keeps GSM8K-50 within budget (`0.7800` dense flexible exact match to
`0.7800` SR24 flexible, strict `0.7400`) but only reaches bs64 math_reasoning
`1.037x` total and `1.049x` full-batch throughput:
...
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_nearfull_padding_prefix2_bs64_probe_20260702
```

Do not use `SPECLINK_SR24_SCHEDULER_POLICY_DENSE_BYPASS=1` for this path; the
bs64 probe dropped to `0.915x` total / `0.973x` full-batch. Do not broaden
near-full tolerance to 8 by default; extra dummy rows lowered speed to `1.030x`
total / `1.039x` full-batch. The next credible implementation target is a
dependency-safe grouped verifier queue or a fused packed MLP operator that can
coalesce bs8/16/32 verifier blocks to effective bs64 without dummy-row waste.
`SPECLINK_SR24_FIXED_BLOCK_CAPACITY_ZERO_DUMMY=0` is the default because dummy
rows are discarded and MLP rows are independent, but the no-zero probe was
neutral (`1.034x` total / `1.048x` full-batch), so do not spend more time on
dummy zero-fill micro-optimization unless a later kernel actually consumes
dummy rows.
`SPECLINK_SR24_ROUTE_OVERLAP_ALLOW_CUDAGRAPH=1` also does not solve this path:
the bs64 probe remained `profile:{"FULL":44,"PIECEWISE":44}` and regressed to
`0.943x` total / `0.991x` full-batch. Keep it off for clean serving claims.
`SPECLINK_SR24_TRITON_ROUTE_ASSEMBLY=1` is also negative on the current
near-full prefix2 path: the bs64 probe dropped to `0.932x` total and `0.985x`
full-batch even though the trace still had 324 mixed opportunities with average
dense/base rows around `144/289`. Keep assembly in the fixed-block PyTorch path
unless the whole mixed MLP is replaced by a fused/grouped kernel.

2026-07-02 down-proj fixed-block operator update: `vllm/vllm/speclink_sr24.py`
now has a gated fixed-block down route controlled by
`SPECLINK_SR24_ROW_ROUTED_DOWN_FIXED_BLOCK`; keep it default-off (`0`). The
down-proj-only microbench shows why: bs64/prefix2 has local headroom only when
dense-important and 2:4-sparse branches run in parallel (`parallel/dense graph
time 0.816x`), but the live Python stream-overlap version regresses to
`0.894x` total throughput and lower GPU utilization. The default-off regression
recovers the current generic path (`1.001x` total, `1.020x` full-batch at
bs64/math/max256). Dense-fill is also not the answer for too few important rows:
the dense-fill=128 probe either degenerates to all-dense at bs8 or leaves tiny
sparse branches at bs16/32 and is slower. Use these roots as the current evidence:
...
For the next optimization attempt, prioritize a grouped verifier queue or a real
fused CUDA/Triton mixed operator. The target data format remains fixed
`[request, K + 1, hidden/intermediate]` blocks plus a compact route descriptor;
important rows should be dense-only and unimportant rows 2:4-only. Low-batch
underfill should be solved by coalescing verifier blocks to an effective bs64
shape or by dense bypass, not by promoting many unimportant rows to dense.

2026-07-02 operator-guard recheck: down-only scopes are not enough for the
1.2x target. The Amdahl ceiling estimate at
...
shows `down_front14` has only a `1.178x` free-op ceiling and `down_front13`
only `1.190x`; broad `all_mlp` coverage is required. The current
`lossy_prefix2_rowrouted_mlp_operator_guard` plus front28 no-verify dense guard
passes GSM8K-50 within the 8pp budget (`0.7800 -> 0.7400`) at
...
but the dense-bypass throughput sanity at
...
is still below dense on total tok/s for bs8/16/32/64 and only near dense on
full-batch tok/s. The Python-stream overlap bs64 sanity at
...
is also negative (`0.894x` total, `0.952x` full-batch). Treat stream overlap as
a rejected ablation. A follow-up serial `packed_parallel` diagnostic using
`SPECLINK_SR24_SCHEDULER_POLICY_ALLOW_SERIAL_PACKED_PARALLEL=1` at
...
is also slower (`0.878x` total, `0.885x` full-batch), even though it executes
49 single-block mixed steps. Keep that flag default-off and diagnostic-only.
The grouped-operator demand analyzer at
`examples/evaluate/eval-guidellm/scripts/analyze_sr24_grouped_operator_need.py`
summarizes why policy selection alone is not enough. On the serial-packed bs64
trace, the grouped fallback rows have an optimistic local operator-only
speedup of `1.241x`, but even `near_full_tolerance=8` covers only `29.8%` of
the operator-unimplemented row weight; the other `70.2%` needs a real
cross-step grouped verifier queue or larger fused shape:
...
The next real implementation needs a dependency-safe grouped verifier queue or
a fused packed MLP operator for `packed_parallel`. New grouping traces also
include `compact_tensor_blocks`, `active_request_verifier_blocks`, and
`target_effective_*` fields so future analysis does not confuse one compact
scheduled tensor with the number of useful request rows inside it.

2026-07-02 grouped queue policy refresh: the prefix2/front28 packed policy at
`examples/evaluate/eval-guidellm/configs/sr24_scheduler_policy_prefix2_front28_packed_k8_effective64.json`
now includes bs8 as `dense_fallback_until_grouped` with
`min_grouped_verifier_blocks=8`, `target_effective_batch_size=64`,
`grouped_dense_rows=192`, `grouped_base_rows=384`, and measured local operator
speedup `1.2398x`. This comes from the packed grouped bucket planner at
...
now accepts `--policy-path` to reanalyze old traces with a refreshed operator
policy. It also writes `queue_plan.csv`, `queue_plan.jsonl`, and
`queue_plan_summary.csv`; those files contain one row per offline
group/fallback decision with `action`, `reason`, `block_indices`, row counts,
fill ratios, and `wait_blocks`. Use that plan as the concrete scheduler replay
input when implementing a live grouped verifier queue. The paired live-shadow
analysis tool is
`examples/evaluate/eval-guidellm/scripts/analyze_sr24_grouped_queue_shadow.py`;
it summarizes `sr24_grouped_queue_shadow_decision` events and can compare them
against `queue_plan_summary.csv`. The current policy-override queue analysis is:
...
It says all batch sizes need a bounded queue, not just policy selection. To
reach `1.2x` local MLP speedup under the optimistic queue model, the minimum
wait budgets are bs8=`15`, bs16=`7`, bs32=`15`, and bs64=`15` blocks on this
math trace. Reads: bs8 is not fundamentally impossible, but it requires
grouping roughly eight useful verifier blocks plus tail fallback; bs16/32/64
also need cross-step grouping to cover drain/tail rows. A live implementation
therefore needs dependency-safe queueing, max-wait timeout, and dense fallback.
The refreshed plan artifact says `max_wait=0` is not viable below bs64
(`1.000x` local MLP estimate for bs8/16/32) and even bs64 reaches only
`1.071x`; `max_wait=15` reaches `1.227x/1.219x/1.203x/1.218x` for
bs8/16/32/64 respectively, but this remains an optimistic local-MLP upper
bound, not an end-to-end throughput claim.

2026-07-02 update for the current 8pp-budget path:
...
`lossy_prefix2_down_front14_compile` is the current boundary candidate:
Llama-3.1-8B GSM8K-50 dense `0.7800` versus SR24 `0.7000` (`-8pp` exactly).
On `math_reasoning`, K=8, max tokens 256, it reaches total speedups
`0.959x/0.950x/1.025x/1.155x` for bs `8/16/32/64` and full-batch speedups
`0.973x/0.987x/1.037x/1.051x`. This is useful evidence but not the 1.2x target.
The broader all-MLP noverify sparse candidate fails badly (`0.7800 -> 0.1400`),
so do not route all non-important tokens sparse-only. The fixed-block
output-buffer local unit check now passes exactly, and a matched GSM8K-20
control shows output-buffer and non-output-buffer `down_front14` both drop
`0.6000 -> 0.5000`. Treat output-buffer as locally assembly-correct; the current
quality loss is from the `down_front14` sparse scope itself. Sample divergence
analysis shows regressions are early/mid reasoning trajectory changes, not final
answer formatting. Do not broaden sparse-only scope until a layer/token risk
gate preserves the 8pp budget on GSM8K limit at least 50.
The follow-up output-buffer gate passed GSM8K-50 with `0.7200 -> 0.6600`
(`-6pp` on that sampled run), but did not improve throughput: math_reasoning
max256 was bs32 `0.889x` total / `1.013x` full and bs64 `1.044x` total /
`1.042x` full. This confirms that allocation/output assembly cleanup alone is
not enough; the main gap is still the split dense/sparse operator and graph
coverage.
The adjacent `down_front13` boundary was also checked manually with
`down_proj=0-12` dense noverify protection. It passed GSM8K-50 in the sampled
run (`0.7800 -> 0.7200`, `-6pp`), but did not improve serving speed:
math_reasoning max256 bs32/64 reached only `0.921x/0.929x` total and
`1.000x/1.007x` full-batch. Treat it as quality-feasible but not a throughput
candidate unless the mixed down_proj operator changes.

The latest prefix0 packed-operator refresh is:
...
Prefix0 makes only the verifier bonus row dense and sends all draft rows through
2:4 sparse. It improves the operator upper bound only when useful rows are
grouped to effective bs64: bs8/16/32 need 8/4/2 grouped verifier blocks
respectively, while bs64 can use a single block. Do not treat prefix0 as a
low-batch live-serving fix by itself; it still requires a dependency-safe
grouping queue and a real packed/fused mixed MLP operator.

2026-07-01 PPoPP-style refresh:
...
The current quality-safe operating point is verifier-only fixed-prefix2:
ordinary/non-verify TLM rows stay dense, verifier prefix/bonus rows are
dense-only, and later verifier draft rows are 2:4 sparse-only. It passes
GSM8K-50 with dense `0.7800` to SR24 `0.7400` (`-4pp`), but throughput is still
parity: bs8/16/32 total speedups are `0.995x/1.006x/1.008x`, and the bs64
64-request补测 is `0.991x` total and `0.996x` full-batch. The more aggressive
noverify sparse-only input-buffer candidate failed badly (`0.7800 -> 0.1600`),
so do not broaden sparse-only scope to all "unimportant" non-verifier tokens.
The refreshed packed-MLP planner still says effective batch around 64 is needed
for local `>=1.2x`: bs8/16/32 require coalescing `8/4/2` verifier blocks in the
same-weight operator microbench. Do not read this as a free live-serving
coalescing plan: cross-step verifier blocks are dependency-sensitive and
cross-layer blocks do not share weights. The current live trace queue upper
bound is:
...
It shows bs8 reaches a local `1.216x` only with max-wait `15`, bs16 reaches
`1.212x` only with max-wait `7`, bs32 stays below target even at max-wait `15`
(`1.185x`), and bs64 reaches `1.221x` only as a long-wait upper bound. Treat
the grouping queue as a diagnostic/design constraint, not the current main
optimization path. The next useful work is to improve the single-block small-M
2:4 sparse operator or replace the split dense/sparse MLP with a true low-
overhead CUDA/CUTLASS grouped/fused kernel; do not spend more time on
threshold/fill/output-buffer tuning or Python-level stream overlap.

Small-M cuSPARSELt alg refresh:
...
For fixed-prefix2 sparse-tail rows, alg1 is better for rows `48/96/192`, while
rows `384` should stay on alg0. The sweep candidate
`lossy_prefix2_verifier_only_outputbuf_smallm_alg1_t256_compile` captures that
with `--sr24-cslt-small-m-threshold 256`. It keeps the same GSM8K-50 quality
(`0.7800 -> 0.7400`) and slightly improves bs32 full-batch tok/s
(`2266.570 -> 2310.436`, `1.019x`), but total tok/s is still `0.990x` and bs64
is `0.995x/0.995x` total/full. Keep the threshold-256 alg knob as a small
operator-selection improvement, but it is not the requested 1.2x solution.

2026-07-01 cache/input-buffer status: SR24 and non-SR24 vLLM processes now use
separate `VLLM_CACHE_ROOT` fingerprints in both the GuideLLM matrix runner and
the lm-eval accuracy runner. This is required because vLLM's default compile
cache key does not include `SPECLINK_SR24_*` env branches; otherwise dense
baselines can replay SR24 graphs and fail with missing SR24-only attributes.
`SPECLINK_SR24_FIXED_BLOCK_INPUT_BUFFER=1` reuses fixed-block dense/base input
buffers and is a valid candidate flag, but it is not yet a general win. Clean
math_reasoning, Llama-3.1-8B, K=8, max128, 64 requests:
...
Input-buffer speedups versus dense are bs8 `1.114x/1.004x`, bs16
`1.053x/0.957x`, bs32 `0.907x/1.202x`, and bs64 `1.105x/1.170x` for
total/full-batch tok/s. It passes GSM8K-50 (`0.7400 -> 0.7400`), but it still
misses the requested `>=1.2x` total-throughput target for most batch sizes.
Use it only as a current data-format candidate; the next optimization should
target grouped/fused useful-row coalescing and total TPS, not another
lossless/threshold sweep.

For diagnostic breakdown rows, do not combine `--sr24-breakdown` with
`--sr24-default-vllm-compile`: the breakdown lock is not TorchDynamo-friendly.
Use graph-only diagnostic mode and enough `--gpu-memory-utilization` for KV
cache. The latest input-buffer bs64 diagnostic is:
...
It shows the sparse base branch remains the main measured cost:
`1.115 ms` per sparse Linear call on average, with gate/up at `1.191 ms` and
down at `1.040 ms`; fixed-block dense/base rows are about `89/178` per call.

2026-07-01 guardrail: do not use Python-side sparse/dense CUDA stream overlap
as a serving optimization. The clean input-buffer overlap probe at:
...
regressed to bs32 `0.431x` and bs64 `0.616x` total-throughput speedup. Any real
pipeline/overlap should be implemented inside a graph-captured grouped/fused
operator, not with Python auxiliary streams in the hot path.

2026-07-01 runtime layer-scope guardrail:
`SPECLINK_SR24_RUNTIME_BASE_ONLY_LAYER_IDS_BY_LEAF` /
`--sr24-runtime-base-only-layer-ids-by-leaf` is diagnostic only. Do not combine
it with `--sr24-default-vllm-compile`; the lm-eval and GuideLLM runners now
reject that combination because vLLM's shared decoder-layer graph can specialize
the runtime branch to the wrong layer. It also failed the current 8pp quality
budget in eager paired smoke: `down_proj=31` dropped GSM8K-20 from dense
`0.7000` to SR24 `0.5500` in:
...
2026-07-01 update for the current optimization target: a fixed-block
dense-fill ablation was added to test whether too-few important rows are the
main limiter. `SPECLINK_SR24_ROW_ROUTED_MLP_FIXED_BLOCK_DENSE_FILL=1` lets the
fixed-prefix row-routed MLP promote adjacent sparse/base rows into the dense
branch until `SPECLINK_SR24_ROW_ROUTED_MLP_MIN_DENSE_ROWS` is reached. This is
only a tile-fill probe: it preserves the same important-prefix semantics, but
it intentionally spends dense work on some low-priority rows to improve dense
Tensor Core occupancy. Keep it off unless explicitly testing fill-factor.

Latest fill-factor gate:
...
Both dense-fill64 and dense-fill128 pass GSM8K-50 exactly under the 8pp budget
(`0.7400 -> 0.7400`), but they are not the target speed path. Dense-fill64
reaches at best `1.026x` total throughput and `1.035x` full-batch throughput;
dense-fill128 reaches at best `1.037x` total throughput and `1.001x`
full-batch throughput. bs64 regresses. A current overlap probe with the same
fill64 data path is also negative: bs32 total/full-batch speedups are
`0.997x/0.979x`, and bs64 total/full-batch speedups are `0.950x/0.949x`.
The conclusion is that fill-factor and Python-side auxiliary stream overlap do
not close the gap. The next credible implementation change is still a
graph-stable grouped/fused mixed MLP operator or a fixed-capacity route-table
kernel that avoids fragmented sparse-base launches and per-row assembly.

Current-tree clean recheck:
...
This short bs64/math/max64 run reruns `dense_baseline`, `base_only_24`,
`all_corrected_24`, and `speclink_t08` without diagnostic CUDA-event timing.
It confirms the active diagnosis after the dense-fill changes:
...
Read: `base_only_24` is not slow because accepted length is low; its full-batch
upper bound is still real when graph coverage is good. `all_corrected_24` is
already using the dense fastpath exact control in this row, so the remaining
exact-path work is to avoid SR24 graph/hook overhead or replace the non-fastpath
residual operator. `speclink_t08` remains only `1.037x` full-batch here and is
not close to the `1.2x` goal; its next lever is the grouped/fused mixed MLP
operator and graph-stable useful-row fill, not another scalar threshold.

For packed-MLP microbench rows, derive the live planner hints with:

```bash
conda run -n spec python examples/evaluate/eval-guidellm/scripts/derive_sr24_packed_mlp_planner.py \
  examples/evaluate/eval-guidellm/results.bak/sr24_packed_grouped_bucket_k8_bs8_64_prefix12_20260701/summary.csv
```
...
It confirms the useful-row fill rule: for K8/prefix2, effective batch around 64
is the first stable local-operator target. bs8 needs coalesce 8, bs16 needs 4,
bs32 needs 2, and bs64 is ready with coalesce 1 before the mixed MLP reaches
the local `1.2x` target. Use `mixed local speedup` for the operator decision;
`mixed serial speedup` is only an optimistic scheduler upper bound. Without
real useful-row grouping, the live planner should use dense fallback rather
than launching underfilled sparse/dense branches.
...
This scan tests prefixes `0..8` at bs8/16/32/64. Promoting extra
low-priority rows into the dense branch does not make bs8/16/32 locally
profitable: best single-block speedups are `0.970x`, `0.975x`, and `0.993x`.
bs64 is the first single-block case where sparse-heavy mixed MLP is locally
profitable (`1.383x` at prefix0, about `1.241x` at prefix2). The current serving
guard preset is:
...
It keeps fixed-prefix2 row-routed MLP semantics, sets
`SPECLINK_SR24_ROW_ROUTED_MLP_MIN_DENSE_ROWS=0`, and requires
`SPECLINK_SR24_ROUTE_MIN_BASE_ROWS=384` before using the mixed branch. Treat
this as an underfilled-operator guardrail only. A tiny throughput-only sanity
under
`results.bak/throughput/lossy_prefix2_rowrouted_mlp_operator_guard_compile/`
gave bs32 `1.023x` and bs64 `0.972x` total tok/s with only 8 requests and
`max_tokens=128`, so it is not a final speedup claim.

2026-07-01 fixed-prefix route descriptor update:
...
`vllm/vllm/speclink_sr24.py` now carries an optional
`FixedPrefixRouteDescriptor` in `VerifyResidualPlan` with
`active_count/scheduled_width/valid_width/prefix/dense_width/base_width`.
Existing residual/base row tensors are still produced. The row-routed
fixed-block MLP consumes the descriptor when it matches the row counts, and the
diagnostic smoke confirmed `row_routed_mlp_fixed_block_descriptor_hits=320` with
zero misses and CUDA Graph FULL decode. This is a live data-format step toward
fixed-capacity route slots and a grouped/fused MLP operator; it is not a
throughput claim by itself.

2026-07-01 descriptor-only fixed-prefix route update:
...
Use the runner flag `--sr24-fixed-prefix-route-descriptor-only`, not a bare
shell `SPECLINK_SR24_FIXED_PREFIX_ROUTE_DESCRIPTOR_ONLY=1`, because the matrix
runners scrub and rebuild `SPECLINK_SR24_*` for the vLLM server. The flag is
wired through both
`examples/evaluate/eval-guidellm/scripts/run_llama31_vllm_fastdraft_smurfs_eagle3_matrix.py`
and `examples/evaluate/eval-guidellm/scripts/run_lm_eval_accuracy.py`; the
`lossy_prefix2_rowrouted_mlp_operator_guard_compile` sweep candidate enables it
for both throughput and quality. The smoke confirmed
`scheduler_fixed_prefix_route_descriptor_only_builds=12`,
`scheduler_fixed_prefix_route_descriptor_only_plan_hits=12`, and
`scheduler_fixed_prefix_route_descriptor_only_skip_bucket=12`, while
`cached_residual_rows_available_steps` and `cached_base_rows_available_steps`
were absent. Treat this as a data-format/scheduler-overhead improvement only:
the remaining 1.2x target still requires a graph-stable grouped/fused MLP or
coalescing path that uses the descriptor to fill real useful rows.

2026-07-01 fixed-block reuse-base ablation:
...
`SPECLINK_SR24_ROW_ROUTED_MLP_REUSE_BASE_OUTPUT=1` now works with fixed-prefix
descriptor-only plans and CUDA Graph capture; the smoke saw
`row_routed_mlp_fixed_block_reuse_base_output_calls=320` and no cached row
tensors. It is a negative ablation, not the recommended serving path. In the
clean bs32/64 math128 check, disjoint fixed-block beat reuse-base at both
batch sizes: bs32 disjoint total/full speedup was `1.255x/1.075x` versus
reuse-base `1.242x/1.049x`; bs64 disjoint was `0.717x/0.962x` versus
reuse-base `0.593x/0.866x`. Do not use reuse-base as the main route; duplicating
sparse work on dense-important rows does not solve the steady-state operator
gap.

2026-07-01 small-M cuSPARSELt follow-up:
...
`lossy_prefix2_noverify_sparse_smallm_alg1_t160_compile` extends the
`alg_id=1` override from rows `<=96` to rows `<=160`, because the K8 operator
sweep showed rows around `144` prefer alg1. It passes GSM8K-50 with no
regression (`0.7400 -> 0.7400`) and improves low/mid batch slightly:
...
Read: alg selection is worth keeping as a small planner knob for underfilled
cuSPARSELt calls, but it is not the main path to `1.2x`. Do not spend more time
on scalar Triton or alg_id-only sweeps unless they are part of a grouped/fused
operator design.

2026-07-01 per-leaf min-base planner probe:
...
`lossy_prefix2_noverify_sparse_gateup384_compile` keeps the current
quality-safe fixed-prefix2 policy but sets
`SPECLINK_SR24_ROUTE_MIN_BASE_ROWS_BY_LEAF=gate_up_proj=384`. This makes
underfilled gate/up sparse branches fall back to full dense while leaving other
leaves on the global min-base rule. It passes GSM8K-50 exactly
(`0.7400 -> 0.7400`). Throughput on math_reasoning/max512:

| bs | total speedup | full-batch speedup | accepted draft/step | avg GPU util |
|---:|---:|---:|---:|---:|
| 32 | `1.022x` | `1.003x` | `2.492` | `94.7%` |
| 64 | `1.216x` | `0.952x` | `2.506` | `93.3%` |

Read: the per-leaf dense fallback can improve total/request-tail behavior at
bs64, but it still does not improve stable full-batch operator throughput.
The current fixed-block row-routed MLP already makes important rows dense-only
and unimportant rows 2:4 sparse-only; the remaining speed gap is branch
fragmentation, activation/assembly overhead, and graph-stable useful-row fill.
Treat `route_min_base_rows_by_leaf` as a planner guardrail, not the final
`1.2x` solution.

2026-07-01 8pp down-only/prefix sweep:
...
This sweep tested the current lossy target directly: GSM8K-CoT `limit=50`,
`max_new_tokens=512`, math_reasoning throughput bs32/64, K=8, and 8pp accuracy
budget. All six candidates passed quality exactly (`0.7400 -> 0.7400`),
including all-MLP prefix2/1/0 and down-proj-only prefix2/1/0 variants. The best
steady full-batch speedup was only `1.001x`; the best total speedup was
`1.320x` from `lossy_prefix2_down_only_noverify_sparse_compile` at bs64, but
that row's full-batch speedup was `0.997x`. Read the total gain as
request-window/tail behavior, not as a stable operator win. Reducing dense
important draft rows from prefix2 to prefix1/prefix0 also does not improve
steady throughput. Future work should stop treating important-row count as the
main blocker and should instead implement a graph-stable grouped/fused mixed
MLP operator that can make gate/up 2:4 profitable without fragmented
gather/sparse/dense/assembly launches.

2026-07-01 Triton assembly and all-corrected current-tree checks:
...
Combining gateup384 with `SPECLINK_SR24_TRITON_ROUTE_ASSEMBLY=1` is negative:
bs32 total/full speedups are `0.963x/0.995x`; bs64 total/full speedups are
`1.211x/0.954x`. The final output assembly kernel is therefore not the main
fixed-block bottleneck. For `all_corrected_24`, a current
`compressed_dense@cuda` no-fastpath gate/up-only check reports
`sr24_residual_device_counts={"cuda:0": 32}`,
`sr24_compressed_residual_runtime_on_gpu=True`, and
`sr24_residual_cpu_module_count=0`, but throughput is still only `0.279x`
total and `0.322x` full-batch versus dense. Enabling default vLLM compile for
that no-fastpath exact path fails at engine startup because Inductor emits dense
`extern_kernels.mm` on a sparse semi-structured gate/up weight and hits
`data is not allocated yet`. Do not rerun Triton route assembly or default
compile no-fastpath `all_corrected_24` as optimization paths unless the sparse
operator is wrapped as a graph-safe opaque/fused op.

2026-07-01 compile/data-format update:

```text
SR24_SYSTEM_OPTIMIZATION_STATUS_20260701.md
```

The latest pass confirms three guardrails for the current lossy SR24 target.
First, full MLP `base_only_24` is still a real sparse upper bound on bs64
(`1.384x` total, `1.522x` full-batch), but GSM8K-50 drops from `0.72` to
`0.22`, so it remains an upper bound only. Second, a layer-scoped
gate/up split (`gate_up_proj` 16-25 corrected, 26-31 base-only) passes
GSM8K-50 in enforce-eager mode (`0.72 -> 0.72`), but default vLLM compile can
reuse a dense decoder-layer graph for sparse-storage layers and emit dense
`extern_kernels.mm` on sparse weights. Do not use layer-heterogeneous SR24
formats with default compile unless graph keying or the operator format is
fixed. Third, a homogeneous channel-pair split avoids that compile issue, but
only the 75% dense-channel variant passes quality (`0.72 -> 0.68`), and it is
slower than dense on bs64/math/max256 (`0.799x` total, `0.852x` full-batch).

Current direction: keep the 8pp/GSM8K-50 quality gate, but stop treating scalar
thresholds, dummy padding, channel fractions, or layer-heterogeneous compile as
the main path. The next implementation needs a uniform graph-stable data format,
fixed-capacity route descriptors, useful-row grouping, dense fallback for
underfilled branches, and a captured grouped/fused dense-important plus
2:4-sparse MLP operator.

Active SR24 goal progress is summarized in:

```text
SR24_ACTIVE_GOAL_PROGRESS_20260630.md
```

Additional 2026-06-30 guardrails:

- `base_only_24` slow rows are not acceptance-length limited when graph/config
  are healthy; accepted draft tokens per step are no worse than dense. Treat
  base-only as the sparse upper-bound signal, not as a quality-safe endpoint.
- `all_corrected_24` optimized exact behavior is the default densefastpath,
  because all-corrected reconstruction is algebraically equal to dense Linear.
  No-fastpath all-corrected rows are operator diagnostics only.
- `compressed_dense` residual execution can be GPU-resident and still slow. Do
  not diagnose it as CPU execution unless `sr24_residual_device_counts` says so.
- The runner's compressed-dense auto cache+prewarm fastpath now requires an
  explicit layer scope. A leaf-only scope such as `gate_up_proj` still covers 32
  large Llama layers and can OOM if all residual dense tensors are prewarmed.
- The current `_compressed_residual_matmul_kernel` Triton packed-values path is
  not a speed path: the gate/up sweep at
  `results.bak/sr24_goal_compressed_residual_triton_sweep_gateup_20260630`
  is about `5x` slower than materialized dense residual GEMM. Use it only as a
  diagnostic unless a new kernel design replaces it.
- The `compressed_dense` chunking ablation also does not rescue
  `all_corrected_24`. With gate_up-only, bs64, max64,
  `residual_out_chunk=0`, densefastpath disabled, residual data on CUDA, and
  no cache/prewarm, the run at
  ...
  reached only `510.243` total / `864.218` full-batch tok/s versus dense
  `1789.089` / `2602.309`. The stats showed
  `sr24_compressed_residual_runtime_on_gpu=True` and `compressed_dense`
  residual modules on `cuda:0`, so this is not CPU fallback. The two-pass exact
  operator itself is the issue.
- Packed verifier-block MLP microbenches show that double-stream dense/sparse
  branches can beat dense only when there are enough rows. K=12/prefix1 reaches
  about `1.20x` standalone at bs32 and `1.24x` at bs64; K=24 makes bs16 nearly
  viable standalone but collapses live EAGLE3 acceptance to about `6%`, so it is
  not a good serving default.
- Live K12/K24 checks did not convert the standalone MLP gain into serving
  throughput. K12/prefix1/overlap on `math_reasoning` reached only about
  `0.65x` dense total throughput at bs32/64. K24 was worse and bs64 failed under
  the current memory budget.
- Do not assume `--no-sr24-noverify-dense-mlp-fastpath` is enough to make
  noverify/non-critical rows sparse. With `selective_correct_non_draft=1`,
  those rows are still protected by the `full_residual_early_dense` path. This
  version passed GSM8K-50 within the 8pp budget (`-6pp`) but stayed slow.
- Fully disabling `selective_correct_non_draft` is too aggressive for the
  current controller: GSM8K-50 fell by `22pp`, outside the 8pp budget. The next
  useful work is a finer non-draft controller or a fused/grouped mixed MLP
  operator, not another global noverify sparse flag sweep.

Current 2026-06-29 read: clean mask build is sub-ms per step, gather/scatter is
small, and GPU utilization is high. The slow path is the current mixed
sparse-base plus residual-correction operator at the residual row fraction
needed for quality. CUDA Graph coverage must be preserved, but dynamic graph
coverage alone has not made `speclink_t08` faster than dense.

2026-06-30 fixed-prefix route-table update: the `lossy_prefix2_rowrouted_mlp`
path now has a contiguous scheduled-block fast path for the common
`[draft K + verifier bonus]` layout. Instead of building base rows as a global
GPU complement (`sort + Triton complement kernel`) it directly constructs
residual rows as `start + {0..prefix-1, bonus}` and sparse/base rows as
`start + {prefix..valid-1}` when request blocks are contiguous. This is the
current code-level implementation of the user-requested data format where
important rows go directly to dense and unimportant rows go directly to 2:4
sparse, without doing dense correction for rows already handled by sparse.

Diagnostic evidence:

```text
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_noverify_dense_mlp_fastpath_bs64_math128_20260630
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_contiguous_route_noverify_mlp_bs64_math128_20260630
```

On Llama-3.1-8B `math_reasoning`, bs64, K=8, max tokens 128 with SR24
breakdown enabled, the route-table CPU time dropped from `72.08 ms/step` to
`0.81 ms/step`; the expensive bucket complement Triton path dropped from `60`
builds to `1` fallback build; and diagnostic full-batch throughput improved
from `1459.261` to `2923.536` tok/s. This proves the route data format was a
real scheduler-side bottleneck, but diagnostic tok/s still must not be used for
final throughput claims because CUDA events perturb timing.

Clean serving evidence for the same preset, max tokens 512, no SR24 breakdown:

```text
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_contiguous_route_clean_b8_16_32_64_math512_20260630
```

| batch/concurrency | dense total tok/s | SR24 total tok/s | total speedup | dense full-batch tok/s | SR24 full-batch tok/s | full-batch speedup |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 8 | `1344.885` | `1238.717` | `0.921x` | `1534.191` | `1351.850` | `0.881x` |
| 16 | `2032.953` | `1943.690` | `0.956x` | `2533.254` | `2230.187` | `0.880x` |
| 32 | `2658.371` | `2482.026` | `0.934x` | `3443.243` | `3052.277` | `0.886x` |
| 64 | `2451.919` | `3160.797` | `1.289x` | `4306.540` | `4305.876` | `1.000x` |

Acceptance length is not the blocker in this clean matrix: SR24 accepted draft
tokens/step are comparable or higher than dense (`2.512/2.441/2.628/2.612`
versus dense `2.440/2.454/2.428/2.316` for bs `8/16/32/64`). The remaining
problem is useful-work efficiency: at bs8/16/32 the extra row-routed sparse-base
and dense-row machinery still costs more than the decode-step savings; at bs64
total throughput benefits from fewer steps/drain time, but saturated full-batch
throughput is only parity. Therefore this patch is a necessary data-layout fix,
not the final 1.2x-in-most-cases solution.

The next credible systems path should be framed as an operator/data-layout
problem: fixed-capacity route tables, grouped or fused sparse-base plus
dense-important MLP execution, and enough tile fill when important rows are too
few to use Tensor Cores efficiently. Avoid returning to no-loss threshold sweeps
or small active-only GEMMs unless the implementation also reduces sparse-base
work for rows that are handled densely.

Do not promote the current Triton bucket dense-GEMM/scatter path as the next
speed fix. The bucket16 parameter sweep at
`results.bak/sr24_triton_bucket16_param_sweep_20260629/summary.md` shows the
best gate/up rows are still about `1.41-1.42x` dense and slower than the
existing bucket-delta path. Future speed work should either reduce corrected
rows with paired-accuracy evidence or replace the two-pass sparse-base plus
correction operator with a fused/grouped implementation.

Row-routed exact-down is a useful scaffold but not a finished speed path. The
2026-06-29 microbench at
`results.bak/sr24_row_routed_exactdown_bucket_sweep_current_20260629/summary.md`
finds the best Llama MLP point near `128` corrected rows: exact-down Triton
assemble is `0.864x` dense and the no-final-assemble lower bound is `0.839x`
dense. That is about `1.16x-1.19x`, close but still below a robust `1.2x`
serving target after routing/assembly overhead. Use it as operator evidence;
do not promote it without a fresh serving plus paired-accuracy gate.

The same row-routed exact-down path has a better isolated upper bound only at
larger rows. The rows=1024 sweep at
`results.bak/sr24_row_routed_exactdown_rows1024_sweep_20260629/summary.md`
reaches exact-down Triton `0.771x` dense at bucket 384, roughly a `1.30x`
operator speedup. That does not yet prove a serving win: the all-layer
`dense_rows` live bs128 attempt at
`results.bak/sr24_rowrouted_mlp_bucket384_bs128_math128_20260629/report.md`
failed before serving because SR24 storage was `1.625x` dense and vLLM reported
no available KV-cache memory. Treat this as a capacity/graph-memory constraint,
not as proof that the row-routed operator is incorrect.
...
Latest bs128 slowdown baseline:

```text
examples/evaluate/eval-guidellm/results.bak/sr24_criticalprefix_b16_bs128_breakdown_20260629/seven_part_report/report.md
examples/evaluate/eval-guidellm/results.bak/sr24_allcorrected_exact_clean_bs128_math128_20260629/report.md
```

On Llama-3.1-8B `math_reasoning`, bs128, K=8, max tokens 128,
`base_only_24` is not slow: clean full-batch tok/s is `3871.417` versus dense
`3147.956` (`1.230x`), accepted draft tokens/step is `1.608` versus dense
`1.422`, GPU util is `91.417%`, and CUDA Graph is `FULL=62,NONE=2`. The current
quality-safe `speclink_t08` reaches `3702.182` full-batch tok/s (`1.176x`) and
`2681.520` total tok/s (`1.197x`), with similarly healthy GPU util and graph
coverage. Therefore the remaining gap to a robust `1.2x` is useful-work
efficiency in the mixed sparse-base plus residual-correction path, not
acceptance collapse or idle GPU.

The same bs128 run confirms exact no-fastpath `all_corrected_24` is not a speed
path. Clean full-batch tok/s is `2773.572` versus dense `3367.839`, and the
diagnostic row localizes the cost to sparse base plus correction:
`1.162 ms/call` sparse base and `0.908 ms/call` residual correction overall
(`gate_up_proj`: `1.360 + 1.088 ms/call`, `down_proj`: `0.766 + 0.548
ms/call`). The residual tensors are GPU-resident (`dense_rows` on `cuda:0`,
`compressed_residual_runtime_on_gpu=True`), so do not diagnose this as CPU
residual execution. Exact all-corrected needs a fused/grouped operator or a
dense-equivalent early-dense control; the current separate sparse-base plus
dense-correction path is inherently slower than dense.

Latest lossy-pivot update on 2026-06-29:
...
Allowing up to about 8 percentage points of accuracy loss changes the target,
but it does not make the current bucket selector sufficient. The fast b12
critical-prefix bucket row on Llama-3.1-8B `math_reasoning`, K=8, max tokens
256 is still negative at small batch sizes: total speedups are `0.752x`,
`0.776x`, `0.923x`, and `1.160x` for bs `8/16/32/64`; full-batch speedups are
`0.736x`, `0.790x`, `0.939x`, and `1.142x`. Its GSM8K-50 paired gate is not
within the allowed-loss budget: dense EAGLE3 flexible exact match is `0.7600`,
while b12 `speclink_t08` is `0.5400` (`-22.0 pp`, pair regressions `12`,
pair improvements `1`). Therefore do not promote b12 or smaller global
residual buckets as the next main path. They are useful speed/quality boundary
points only.

The user-requested "do not run dense for rows already handled by sparse" idea
maps to a row-routed data format: important rows should go through dense
directly, and unimportant rows through 2:4 sparse directly, instead of doing
sparse base for every row and then overwriting important rows. Existing
row-routed experiments show this is the right systems direction but not yet a
finished implementation: the live path needs graph-safe fixed-capacity row
plans, enough packed rows to use Tensor Cores efficiently, and a better
per-request/value-aware bucket so useful draft rows are not crowded out.
The quick `--sr24-bucket-dense-compute-active-only` ablation remains a
diagnostic-only switch. The 2026-06-30 active-only run at
`results.bak/sr24_activeonly_vs_activefused_bs64_math128_20260630/` passed the
GSM8K-50 8pp quality budget exactly (`0.72 -> 0.64`) but serving throughput was
only `0.381x` total / `0.511x` full-batch versus dense. The dynamic
`nonzero()/index_select()` active compaction avoids dense work for unimportant
rows, but it destroys the execution shape and is not a speed path by itself.

The 2026-06-30 graph-safe fixed-layout ablation
`--sr24-bucket-dense-active-mask-fused` is the best current small step, but it
is not yet a final solution. With all MLP leaves, direct cuSPARSELt, prefix2,
bucket32, active-mask fused GEMM+scatter, and K=8 on Llama-3.1-8B
`math_reasoning` bs64:

- GSM8K-50 `speclink_t08` accuracy is `0.66`, within the 8pp budget versus the
  dense reference `0.72`.
- max128 reaches `2169.181` vs dense `2258.660` total (`0.960x`) and
  `3634.799` vs `3026.842` full-batch (`1.201x`).
- max512 reaches `2907.552` vs dense `2482.865` total (`1.171x`) and
  `5240.547` vs `4426.090` full-batch (`1.184x`).
- max2048 reaches only `2520.563` vs dense `2427.632` total (`1.038x`) and
  `4681.783` vs `5386.383` full-batch (`0.869x`), with KV-cache pressure near
  full.

The active-mask fused breakdown at
`results.bak/sr24_activefused_breakdown_bs64_math128_20260630/` shows why this
is not enough: CUDA Graph coverage is mostly healthy (`FULL=62,NONE=2` in the
diagnostic run), and fused dense correction is small (`7.6 ms` total), while
2:4 sparse base Linear dominates (`148.4 ms` total) and scheduler/bucket CPU is
secondary (`scheduler_mask_build_cpu_ms` about `35.8 ms`, bucket top-k about
`6.3 ms`). A direct-position bucket probe at
`results.bak/sr24_activefused_directpos_bs64_math512_20260630/` was slightly
slower than the normal active-mask fused run, so top-k replacement alone is not
the next main lever.

Latest 2026-06-30 long-output follow-up: the same active-mask fused direction
still passes the 8pp GSM8K-50 budget (`0.72 -> 0.66`) but does not reach the
serving target at max tokens 2048. The root
`results.bak/sr24_activefused_mlpall_prefix2_bs64_math2048_20260630_032223/`
shows bs64 total speedup `1.049x` and full-batch speedup `0.889x`. A down-only
controller is accuracy-safe on GSM8K-50 (`0.72 -> 0.72`) but lowers accepted
draft tokens/step and is also slower at bs64/max2048
(`results.bak/sr24_downall_prefix2_bucket32_bs64_math2048_20260630_032223/`).
The low-storage gate_up split remains the best long-output total-throughput
point at bs64 (`1.345x` total) but is slow for bs8/16/32 and still below dense
in the full-batch window:
`results.bak/sr24_gateup_res16_25_base26_31_critical4_b8_16_32_64_math2048_20260630/`.
The current summary is in `SR24_LOSSY_OPTIMIZATION_STATUS_20260630.md`.

Latest tile-fill data-format probe: `--sr24-preset
mlpall_tilefill_prefix2_bucket32_cublas` is available in both throughput and
lm-eval runners. It uses all MLP leaves, low-confidence prefix2 correction,
direct cuSPARSELt sparse base, fixed bucket32 dense overwrite, and cuBLAS dense
bucket correction. This is the preferred reproduction for testing whether
important-token groups should be padded into larger stable tiles instead of
using active-only tiny GEMMs. On GSM8K-50 it matched dense in aggregate
(`0.72 -> 0.72`). On Llama-3.1/math_reasoning/bs64/K8/max512 it reached
`1.142x` total and `1.190x` full-batch speedup; at max2048 it fell to `1.004x`
total and `0.847x` full-batch. Result roots:
`results.bak/sr24_tilefill_bucket32_cublas_prefix2_gsm8k50_quality_20260630/`,
`results.bak/sr24_tilefill_bucket32_cublas_prefix2_bs64_math512_20260630/`,
and
`results.bak/sr24_tilefill_bucket32_cublas_prefix2_bs64_math2048_20260630/`.
Treat fixed tile fill as the current promising data-layout direction, but not
as a finished max2048 serving solution.

The tile-fill follow-up breakdown is under
`results.bak/sr24_tilefill_bucket32_cublas_breakdown_bs64_math128_20260630/`.
In the diagnostic row, sparse base dominates (`1.173 ms/call`) while dense
correction is secondary (`0.139 ms/call`), gather/scatter is small
(`0.014 ms/call`), bucket fill is high (`31.162/32`), and draft residual
fraction is still high (`0.816`). Therefore the remaining issue is not empty
important-token tiles; it is duplicated/full sparse-base work for many rows.
Two scalar reductions are negative despite passing GSM8K-50 (`0.72 -> 0.72`):
prefix1 threshold0.6 is only `0.921x` total / `0.904x` full-batch on
bs64/math/max512, and prefix2 threshold0.4 is `0.848x` total / `0.903x`
full-batch. Result roots:
`results.bak/sr24_tilefill_bucket32_cublas_prefix1_gsm8k50_quality_20260630/`,
`results.bak/sr24_tilefill_bucket32_cublas_prefix1_bs64_math512_20260630/`,
`results.bak/sr24_tilefill_bucket32_cublas_prefix2_thr04_gsm8k50_quality_20260630/`,
and
`results.bak/sr24_tilefill_bucket32_cublas_prefix2_thr04_bs64_math512_20260630/`.
Do not spend more time on simple prefix/threshold reductions unless a new
operator path reduces sparse-base work for dense-corrected rows.

The direct disjoint row-routed probe for the user-requested "important rows go
dense, unimportant rows stay sparse" idea is also negative as a current serving
path:

- clean max512 root:
  `results.bak/sr24_disjoint_rowrouted_min1_bs64_math512_20260630/`
- breakdown root:
  `results.bak/sr24_disjoint_rowrouted_breakdown_bs64_math128_20260630/`
- bs64/K=8 `math_reasoning` max512 speed: `1.082x` total and `1.032x`
  full-batch versus dense EAGLE3
- diagnostic row-routed MLP did execute, with `50176` dense rows and `442464`
  base rows, but base 2:4 work still dominated:
  `row_routed_mlp_base_gate_up_cuda_ms=50.49` and
  `row_routed_mlp_base_down_sparse_cuda_ms=38.10`

Do not promote the current row-routed Python/Torch row-list implementation.
It avoids duplicate sparse work for corrected rows, but loses the benefit by
splitting the 2:4 base GEMM into smaller row slices and paying gather/assemble
overhead. The next credible SR24 path is a fixed-capacity grouped route-table
operator.

2026-06-30 follow-up on the user-requested systems direction:

- Two-stream row-routed MLP overlap is a negative ablation. In
  `results.bak/sr24_rowrouted_mlp_overlap_eager_breakdown_bs64_math64_20260630/`,
  overlap did execute (`row_routed_mlp_overlap_stream_calls=1280`) but remained
  about `235` output tok/s, essentially the same as the no-overlap eager
  control in
  `results.bak/sr24_rowrouted_mlp_nooverlap_eager_breakdown_bs64_math64_20260630/`.
  The sparse branch still dominated cumulative time, and stream splitting
  forced the path out of CUDA Graph.
- `--sr24-preset fixedprefix4_all_rowrouted_graph` is now available in both the
  throughput and lm-eval runners. It is a fixed-prefix/non-draft-all route-all
  graph probe that avoids `direct_cpu_route_rows` and uses MLP-level row
  routing so dense rows do not also pay sparse-base work. The runner allows
  CUDA Graph only for this narrow route-all shape. It is graph healthy but not
  fast enough: `results.bak/sr24_fixedprefix4_all_rowrouted_graph_noforceeager_bs64_math512_20260630/`
  reports dense `2818.885` / `4216.231` total/full-batch tok/s versus SR24
  `2562.341` / `3982.950`, with `profile:{"FULL":49}` and SR24 GPU util
  `95.45%`.
- Active-only bucket correction directly tests "unimportant token already ran
  sparse, so skip dense for it." The current Python/Torch active-row compaction
  is a strong negative result:
  `results.bak/sr24_tilefill_prefix2_activeonly_bs64_math512_20260630/`
  reports dense `2819.236` / `4215.199` total/full-batch tok/s versus SR24
  `1178.266` / `1911.627`, with GPU util only `42.51%`. Do not use active-only
  compaction as the main path; preserve fixed-capacity route tables and put the
  active mask inside a fused/grouped kernel instead.

The next credible SR24 path remains a fixed-capacity grouped route-table
operator: graph-stable row metadata, large-enough dense/sparse tiles, fused
assembly, and optional dense/sparse stream overlap only after both branches are
large enough to occupy the GPU.

2026-06-30 same-condition no-verify dense MLP fastpath A/B:
...
With `lossy_prefix2_rowrouted_mlp`, math_reasoning, K=8, max512, and the same
`max_num_batched_tokens=8192` / `gpu_memory_utilization=0.88` setting as the
clean route-table run, disabling `SPECLINK_SR24_NOVERIFY_DENSE_MLP_FASTPATH`
does not reveal a hidden speedup. bs8 falls from `1238.717` to `1226.344`
total tok/s and bs16 rises only noise-level from `1943.690` to `1960.913`;
both remain below dense. Keep the fastpath enabled by default because it
prevents non-verify/no-mask decode work from entering the expensive per-Linear
SR24 path, but do not treat it as the remaining blocker.

2026-06-30 lossy system candidate gate:
...
The `gateup_res16_25_base26_31_critical4_smallrow160` candidate tests a
systems-flavored policy: residual-correct only `gate_up_proj` layers 16-25,
leave 26-31 sparse-only, and fall back to dense for no-residual `gate_up_proj`
steps with <=160 rows to avoid tiny sparse kernels. It passes the current
GSM8K-50 lossy gate exactly at the allowed boundary: dense `0.7600` vs SR24
`0.6800` (`-8.0 pp`, pair regressions/improvements `5/1`). However it is not
the main throughput path:
...
This confirms the current 8pp-loss direction can buy high-batch total
throughput by shrinking residual storage to `1.25x` dense, but it still fails
the desired bs8/16/32 behavior and the saturated full-batch target. Do not spend
more time on simple layer-scope/smallrow fallback as the main optimization.
The next implementation must reduce base sparse work or fuse/group disjoint
sparse/dense branches; otherwise high GPU utilization only means the GPU is busy
with inefficient work.

Latest 2026-06-30 follow-up:

- Fixed-shape active-mask fused bucket correction restores graph/utilization
  but is still slower than dense under the current Triton kernel. In
  `results.bak/sr24_tilefill_prefix2_active_mask_fused_bs64_math512_20260630/`,
  Llama bs64/math/max512 SR24 reaches `2580.280` total and `3810.455`
  full-batch tok/s versus dense `2819.112` and `4217.142`, with GPU util
  `93.10%` and `profile:{"FULL":49}`. The microbench at
  `results.bak/sr24_triton_bucket_param_sweep_gateup_down_bs64_20260630/`
  shows the current Triton bucket dense-GEMM/scatter still takes about
  `0.76 ms` for `gate_up_proj` and `0.29 ms` for `down_proj` at bucket32 and
  is nearly unchanged between residual fractions `0.0625` and `0.125`; it
  fixes shape, not useful-work count.
- The current fixed tile-fill control also did not reproduce the older max512
  positive result: `results.bak/sr24_tilefill_prefix2_current_bs64_math512_20260630/`
  reports `0.922x` total and `0.888x` full-batch versus dense. Treat the older
  `1.142x/1.190x` max512 tile-fill number as stale unless a fresh same-condition
  run reproduces it.
- Coarse late-layer base-only is a speed upper-bound, not a valid quality path.
  In `results.bak/sr24_lossy_tail16_baseonly_speed_bs64_math512_20260630/`,
  `base_only_24` on `gate_up_proj=16-31;down_proj=16-31` reaches about
  `1.38x` dense throughput at bs64/max512. But
  `results.bak/sr24_lossy_tail16_baseonly_gsm8k50_quality_20260630/` shows
  GSM8K-50 exact match drops from `0.72` to `0.22` (`-50 pp`), far beyond the
  allowed `8 pp` loss. Do not pursue broad pure base-only ranges as the main
  lossy route.
- `run_sr24_baseonly_scope_sweep.py` now resolves `--output-root` and
  `--work-root` to absolute paths before launching child commands; this avoids
  nested `eval-guidellm/examples/evaluate/eval-guidellm/...` outputs when the
  parent script starts subcommands from `eval-guidellm/`. The focused 2026-06-30
  scope gate at
  `results.bak/sr24_baseonly_scope_gate_quality50_speed64_20260630_continue2/`
  shows why layer-scope-only tuning is not enough: `gate_up_proj=16-31` passes
  GSM8K-50 exactly at the `8 pp` budget (`0.72 -> 0.64`) but its base-only
  full-batch speed ceiling is only `1.102x`; `down_proj=8-15` fails quality
  (`0.72 -> 0.58`); and `gate_up_proj=31` with `up_sparse` is quality-safe
  (`0.72 -> 0.74`) but slower than dense (`0.967x` full-batch). Do not spend
  more time on simple layer-scope sweeps unless a new mixed operator gives
  corrected paths enough speed headroom.
- `compressed_dense` residual work is GPU-resident, but the current packed
  Triton residual matmul is diagnostic-only. The probe at
  `results.bak/sr24_compressed_residual_kernel_probe_20260630_continue/`
  shows best packed Triton is `7.610x` slower than materialized residual
  torch GEMM for `gate_up_proj` and `9.183x` slower for `down_proj`. For
  current `all_corrected_24` ablations, prefer cached/prewarmed GPU
  materialized residual weights when memory allows; do not enable
  `SPECLINK_SR24_COMPRESSED_RESIDUAL_TRITON=1` for throughput claims.
- 2026-06-30 graph-on base-only update: broad full-MLP base-only becomes a real
  upper bound only when the throughput runner uses
  `--sr24-base-only-allow-compile --sr24-allow-cudagraph`. In
  `results.bak/sr24_baseonly_compile_ab_bs32_64_math256_20260630/`, full-MLP
  base-only reaches `1.167x/1.652x` total speedup at bs32/64, but
  `results.bak/sr24_baseonly_mlpall_gsm8k50_accuracy_20260630/` drops GSM8K-50
  from `0.72` to `0.22`, so it is only a speed ceiling. The current pure
  base-only quality-bound scope is `gate_up_proj=16-31`: the scope gate reports
  `0.72 -> 0.64` (`-8 pp`), and
  `results.bak/sr24_gateup16_31_baseonly_compile_bs8_16_32_64_math256_20260630/`
  gives total speedups `0.959x/0.952x/1.011x/1.206x` for bs8/16/32/64. This
  confirms that quality-safe 2:4 needs a row-count-aware fallback or a fused
  grouped operator before it can meet `>=1.2x` on most batch sizes.
- `run_sr24_baseonly_scope_sweep.py` now passes
  `--sr24-base-only-allow-compile` only to the GuideLLM throughput child
  command. lm-eval quality children remain eager because
  `run_lm_eval_accuracy.py` does not expose this flag and quality gates should
  avoid compile/graph correctness ambiguity.
- 2026-06-30 all-corrected backend check for `gate_up_proj=16-31`, bs64,
  Llama math/max256: `dense_rows` residual is slow (`0.737x` total, graph
  `NONE`, storage `1.625x`); `torch_sparse` residual is the best exact path
  (`1.000x` total, `0.910x` full-batch, graph `FULL`, storage `1.1875x`);
  GPU-resident `compressed_dense` is not a CPU-transfer issue but is still
  slower (`0.836x` total, `0.872x` full-batch, graph `FULL`, storage `1.125x`).
  The compressed run explicitly used cached/prewarmed CUDA residuals with
  `residual_out_chunk=0` and reported no non-GPU residual modules. For current
  exact all-corrected throughput baselines, use `torch_sparse` residual plus
  direct cuSPARSELt; treat `compressed_dense` as a storage/operator experiment,
  not the fastest path.

Next SR24 work should be systems-first rather than another scalar threshold
sweep:

- Data format: keep fixed-capacity per-step/per-position row buckets with
  active masks and request ids, so CUDA Graph sees stable pointers and shapes.
- Operator: implement a row-routed/fused MLP or grouped Linear plan where dense
  rows and sparse rows are disjoint; avoid sparse-base work for rows that will
  be dense-corrected.
- Occupancy: if important rows are too few, aggregate across requests/layers or
  use a measured fallback threshold; do not launch tiny dense/sparse kernels
  just to preserve theoretical FLOPs.
- Parallelism: test two-stream sparse-row and dense-row execution only after the
  row counts are large enough; otherwise stream and synchronization overhead can
  dominate.
- Quality gate: use GSM8K limit 50 or larger for lossy candidates, and allow
  at most about 8 pp absolute loss before expanding to broader datasets and
  bs `8/16/32/64`.
...
- SpecLink SR24 selective residual 2:4 experimental support in
  ...
  `SPECLINK_SR24_ADAPTIVE_DENSE_FALLBACK_NO_RESIDUAL_ONLY=1` restricts the
  fallback planner to sparse-only/no-residual steps, and
  `SPECLINK_SR24_ADAPTIVE_DENSE_FALLBACK_SMALL_GATE_UP_NO_RESIDUAL=1` permits
  the same small-row fallback for gate/up no-residual modules. The gate/up
  flag does not fire unless those modules also keep a dense weight, which
  raises storage and was negative in the 2026-06-30 low-batch probe.
...
SPECLINK_SR24_GROUPED_QUEUE_SHADOW=0
SPECLINK_SR24_GROUPED_QUEUE_MAX_WAIT_BLOCKS=0
...
- 2026-06-29 lossy/system-design pivot:
  the target is now explicitly allowed to lose up to 8 percentage points of
  accuracy, so do not keep requiring paired no-loss before looking at speed.
  However, the new small-scale probes show that selection alone is not enough.
  All result roots are under `examples/evaluate/eval-guidellm/results.bak/`:
  `sr24_lossy_mlpall_prefix5_tritonoverride_speed_b8_16_32_64_20260629`,
  `sr24_baseonly_upperbound_speed_b8_16_32_64_20260629`,
  `sr24_route_overlap_fixedprefix_bs64_math128_20260629`, and
  `sr24_baseonly_dense_nonverify_speed_b8_16_32_64_20260629`.
  On Llama-3.1-8B/math_reasoning/K8/max128, all-MLP prefix5 + Triton override
  is negative across bs8/16/32/64 (`0.490x/0.516x/0.659x/0.712x` total
  speedup). Pure `base_only_24` is also negative for bs8/16/32
  (`0.622x/0.589x/0.735x`) but reaches `1.185x` total and `1.848x`
  full-batch at bs64, proving the sparse upper bound exists only when enough
  verifier rows are packed. `SPECLINK_SR24_ROUTE_OVERLAP_STREAMS=1` was added
  as an explicit routed dense_rows ablation and is wired through both throughput
  and lm-eval runners; it did run (`route_all_residual_rows_overlap_stream_calls=1608`)
  but forced CUDA Graph `NONE` and reached only `0.714x` total at bs64. Keep it
  as an operator-design probe, not a default. `base_only_24
  --sr24-base-only-dense-nonverify` OOMs in the full-model setup because dense
  non-verify copies plus 2:4 storage exceed 32GB; dense fallback must be
  layer/module scoped. The next serious path should be a graph-safe fixed-layout
  mixed MLP: fixed route slots `{row_id, route_kind, active}`, no sparse work
  for important dense rows, no dense work for unimportant sparse rows, and
  pre-created stream overlap or a fused packed operator that avoids Python-side
  dynamic `nonzero()` and CUDA Graph loss.
- 2026-06-29 scoped fallback and current SR24 candidate status:
  `SPECLINK_SR24_BASE_ONLY_DENSE_VERIFY_MAX_ROWS` now has independent optional
  filters `SPECLINK_SR24_BASE_ONLY_DENSE_VERIFY_LAYER_IDS` and
  `SPECLINK_SR24_BASE_ONLY_DENSE_VERIFY_LAYER_IDS_BY_LEAF`, wired through the
  throughput and lm-eval runners. Use them only for memory-fitting/base-only
  upper-bound probes. The full tail dense-verify fallback still OOMs at attach,
  while `gate_up_proj=31;down_proj=31` attaches and records two dense-copy
  fallback modules. On Llama-3.1/math/K8/max128, this scoped `base_only_24`
  reaches bs64 total/full-batch speedup `1.141x/1.178x` and bs128
  `1.397x/1.810x`, but GSM8K-50 accuracy is unusable (`0.1600` vs dense
  `0.7400`), so it is not a candidate under the 8pp loss budget.
  The current accuracy gate is still `speclink_t08` with
  `criticalprefix4_bucket16_directcslt`: GSM8K-50 matched dense exactly at
  `0.7400/0.7400`, but the max128/math speed matrix is only
  bs8/16/32/64 total speedup `0.701x/0.748x/0.855x/0.959x` and full-batch
  `0.706x/0.766x/0.877x/1.009x`. A row-routed MLP smoke is directionally
  correct but still insufficient at bs64 (`0.962x` total, `1.024x` full-batch).
  Next optimization should be a graph-stable fixed route-table or fused/grouped
  mixed MLP that avoids duplicate sparse+dense work for important rows, not a
  threshold-only sweep.
- 2026-06-29 row-routed/tile-fill follow-up: results are under
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_ppopp_rowrouted_breakdown_bs64_math128_20260629/`,
  `sr24_rowrouted_bucket64_bs64_math128_smoke_20260629/`,
  `sr24_rowrouted_bucket128_bs64_math128_smoke_20260629/`,
  `sr24_rowrouted_bucket64_instrumented_bs64_math64_20260629/`, and
  `sr24_adaptive_bucket64_bs64_math128_smoke_20260629/`. Bucket64 real
  row-routing confirms important rows avoid duplicate sparse+dense work, but
  the remaining base sparse branch dominates: gate_up base sparse is about
  `0.982ms/call` versus dense selected rows `0.156ms/call`, and down base
  sparse is about `0.626ms/call` versus dense selected rows `0.086ms/call`.
  Bucket64/128 tile filling and adaptive full-layer dense fallback are all
  negative on total tok/s. Under the new 8 percentage-point accuracy-loss
  budget, do not spend more time on bucket-only or threshold-only sweeps unless
  they also show a speed path; the next credible systems direction is a
  graph-stable route-table plus fused/grouped packed mixed operator.
- 2026-06-29 lossy SR24 gate update: the current optimization budget is now
  explicit accuracy loss within 8 percentage points, not paired no-loss.
  Use
  `examples/evaluate/eval-guidellm/scripts/run_sr24_lossy_speed_quality_sweep.py`
  for small candidate gates: it runs GSM8K `limit=50` first and only runs
  throughput for candidates that pass the quality budget. Outputs default to
  `examples/evaluate/eval-guidellm/results.bak/` and work dirs to `temp/`.
  Two bs64/math/K8/max128 probes were run:
  ...
  for `lowresidual_gateup_riskcap2` and
  ...
  for `lowresidual_gateup_riskcap2_rowrouted`. Both passed GSM8K-50
  (`dense=0.7200`, `SR24=0.7200`, `0pp` drop), but speed was insufficient:
  non-row-routed full/total speedup `1.0185x/0.9257x`, row-routed
  `0.9906x/0.9192x`. Interpretation: row-level fidelity split is the right
  dataflow goal, but with tiny dense-row counts the current PyTorch dynamic
  gather + small dense GEMM + scatter path is slower than base-first overwrite.
  Keep the row-routed min-row guard unless a fused/grouped route-table operator
  or profitable tile-fill policy is available.
- 2026-06-30 base-only lossy scope gate: `run_sr24_baseonly_scope_sweep.py`
  now supports `--quality-gate`, `--quality-limit`, and
  `--max-accuracy-drop-pp`. It runs GSM8K `limit=50` first and skips
  throughput for scopes outside the 8pp quality budget. The first small run is:
  ...
  On Llama-3.1-8B/math/K8/max128/bs64:
  `gateup16_31` barely passes quality (`0.72 -> 0.64`, `-8pp`) but reaches
  only `1.068x` full-batch and `0.982x` total speedup; `gateup_all` fails
  quality (`0.72 -> 0.42`, `-30pp`) and is skipped; `gateup31 up_sparse`
  passes quality (`0.72 -> 0.74`) but is slower (`0.966x` full-batch,
  `0.911x` total). The matching gate_up-shaped microbenchmark is
  ...
  pure 2:4 sparse base is about `0.657x` of dense for a 512x28672x4096
  Linear, but base-first dense correction and full routed assembly erase most
  of that gain. Future SR24 work should focus on graph-stable packed route
  tables, fused/grouped mixed operators, and possibly concurrent sparse-base
  plus dense-important execution, not another no-loss threshold sweep.
- 2026-06-30 SR24 operator follow-up: do not treat current compressed residual
  Triton, cached-stream overlap, or exact no-fastpath `all_corrected_24` as
  default speed paths. The packed compressed residual microbench at
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_compressed_residual_kernel_profile_20260630_001736/`
  shows the direct Triton packed residual kernel is `7.614x` to `9.212x`
  slower than dense residual GEMM. The cached-stream overlap serving probe at
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_route_overlap_cached_streams_bs64_math128_20260630/`
  reaches only `0.769x` total and `0.874x` full-batch versus dense. The narrow
  exact `all_corrected_24` torch-sparse residual probe at
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_allcorrected_torchsparse_gateup16_31_bs64_math128_20260630/`
  reaches only `0.812x` total and `0.841x` full-batch even with CUDA Graph
  `{"FULL":62,"NONE":2}`. `SPECLINK_SR24_ROUTE_OVERLAP_STREAMS=1` now reuses
  per-device streams and is useful for ablation, but the next real speed path
  still needs a fused/grouped route-table operator where dense-important rows
  and sparse-unimportant rows are mutually exclusive and graph-stable.
- 2026-06-30 lossy prefix/direct-cuSPARSELt follow-up: the best small-scale
  controller so far is `mlpall_direct_prefix2`, implemented in
  `examples/evaluate/eval-guidellm/scripts/run_sr24_lossy_speed_quality_sweep.py`.
  It uses the `mlpall_lowconf_prefix5_tritonoverride` preset plus
  `--sr24-direct-cslt-linear --sr24-selective-min-prefix-residual 2`, i.e. all
  MLP `gate_up_proj/down_proj` layers use the 2:4 base, corrected rows use
  dense residual fidelity, and only two draft prefix rows are forced dense.
  Results are under
  `examples/evaluate/eval-guidellm/results.bak/sr24_mlp_directcslt_gate_bs64_math128_20260630/`,
  `sr24_mlp_direct_prefix_sweep_bs64_math128_20260630/`,
  `sr24_mlp_direct_prefix2_bs64_math512_20260630/`, and
  `sr24_mlp_direct_prefix2_bs64_math2048_20260630/`.
  On GSM8K-50, `prefix2` stays within the current 8pp accuracy budget
  (`0.72 -> 0.66`, `-6pp`), while `prefix0` fails (`0.72 -> 0.52`). On
  Llama-3.1/math_reasoning/bs64/K8/max128, `prefix2` reaches `1.193x`
  full-batch speedup but only `0.957x` total speedup. At max512 it reaches
  `1.186x` total speedup. At max2048 it falls to `1.020x` total speedup and
  is slower on the full-batch estimate; dense already accepts about `4.01`
  draft tokens/step while `prefix2` accepts `4.18`, so step-count savings are
  too small to hide the mixed sparse/residual operator cost. Do not promote
  this as the final default. The useful code change is the direct-cuSPARSELt
  candidate family; the next speed path still needs a fused/grouped mixed MLP
  operator or graph-stable route table that avoids duplicate sparse work for
  important dense rows and avoids tiny residual GEMMs.
- 2026-06-30 SR24 current systems direction: use the 8 percentage-point
  accuracy-loss budget explicitly, but do not hide low-batch negative speedups.
  `--sr24-preset lossy_static_tail` is an alias for the static-tail speed
  upper-bound candidate (`gate_up_proj=31;down_proj=30-31`, qkv/o exact
  densefastpath), with clearer semantics than `throughput_aggressive`. It is
  not quality-safe.
  The fresh long-output math/K8/max2048 check at
  `examples/evaluate/eval-guidellm/results.bak/sr24_throughput_aggressive_current_math_bs32_64_max2048_20260630/`
  reports bs32 dense/SR24 `3992.809/3850.615` tok/s and bs64
  `2405.836/3964.503` tok/s. The matching GSM8K-50 quality gate at
  `examples/evaluate/eval-guidellm/results.bak/sr24_lossy_static_tail_gsm8k50_quality_20260630/`
  fails badly: dense `0.7200`, SR24 `0.4000` (`-32pp`, 18 paired
  regressions). Do not use broad static tail sparsity as the main lossy route
  under the 8pp budget. Acceptance is higher for SR24, so the remaining speed
  issue in quality-preserving candidates is not accepted-length alone; it is
  the current operator/graph overhead not amortizing below large batch. The
  throughput runner also has
  `--sr24-min-enabled-batch-size`: when set, SR24 methods below that
  client-side concurrency run the dense EAGLE3 server and record
  `sr24_batch_fallback=true` in `summary.csv`. This is only a guard for
  engineering sweeps, not a claim that 2:4 is faster at fallback batch sizes.
  The paper-quality route remains a fixed-capacity grouped route-table or
  fused mixed MLP operator: dense-important rows and 2:4 sparse-unimportant rows
  must be disjoint, graph-stable, and large enough to occupy the GPU; tiny
  active-row GEMMs, bucket-only threshold sweeps, and Python gather/scatter
  paths have already been negative.
- 2026-06-30 current best 8pp-budget live candidate:
  `--sr24-preset mlpall_direct_prefix2` is now available in both the throughput
  and lm-eval runners. It uses all MLP `gate_up_proj/down_proj` layers,
  low-confidence residual routing with mandatory dense prefix2, direct
  cuSPARSELt sparse base, bucket32, and dynamic CUDA Graph. The quality/speed
  gate at
  `examples/evaluate/eval-guidellm/results.bak/sr24_lossy_prefix2_sweep_quality50_bs64_max512_20260630/`
  passes GSM8K-50 within the current budget: dense `0.7200`, SR24 `0.6600`
  (`-6pp`, 6 paired regressions and 3 paired improvements). On
  Llama-3.1/math/K8/max512/bs64 it reaches total/full-batch speedups
  `1.1969x/1.1748x`, close to but not reliably above the 1.2x target. The
  batch sweep at
  `examples/evaluate/eval-guidellm/results.bak/sr24_lossy_prefix2_speed_bs8_64_max512_20260630/`
  shows the current implementation is not a majority-batch solution:
  total speedups are about bs8 `0.570x`, bs16 `0.602x`, bs32 `0.651x`, and
  bs64 `1.181x`; full-batch speedups are bs8 `0.557x`, bs16 `0.600x`,
  bs32 `0.659x`, and bs64 `1.156x`. This means the controller is plausible
  for quality, but the operator/dataflow is the bottleneck below large batch.
  Next work should implement the grouped/fused route-table design instead of
  more scalar threshold sweeps.
- 2026-06-30 fixed-prefix/score-free mixed-shape diagnosis:
  `--sr24-preset mlpall_fixedprefix2_directcslt` is now available in both the
  throughput and lm-eval runners. It removes per-token confidence scoring from
  the hot path and always protects draft prefix2 plus the verifier bonus row,
  while all other draft rows are sparse-only. This is the closest current
  prototype to the desired disjoint route-table semantics: important rows do
  not need the sparse base, and unimportant rows do not need dense correction.
  In the quality/speed gate at
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_fixedprefix2_scorefree_quality50_bs64_max512_20260630/`,
  GSM8K-50 matched dense exactly (`0.7200 -> 0.7200`), and bs64
  Llama-3.1/math/K8/max512 reached total/full-batch speedups
  `1.894x/1.198x`. This proves there is a real sparse upper bound when rows
  are fixed, packed, and graph-covered.
  The same direct cuSPARSELt path is not stable for normal continuous serving
  with mixed prefill/decode shapes: the bs8/16/32/64 sweep at
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_fixedprefix2_scorefree_speed_bs8_64_max512_20260630/`
  had bs8/bs64 successes but bs16/bs32 failures in
  `torch._cslt_sparse_mm`/`vectorized_gather_kernel` during mixed steps such as
  `total_num_scheduled_tokens=222`. For correctness, this preset now sets
  `SPECLINK_SR24_FORCE_CUDAGRAPH_NONE_FOR_MIXED=1`, but that guard is only a
  stability fallback: the guarded run at
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_fixedprefix2_scorefree_mixedguard_speconly_bs16_64_max512_20260630/`
  finished without crashes but dropped to bs16/32/64 total throughput
  `540.757/771.103/1065.975` tokens/s versus dense references
  `2222.948/2704.280/2481.339`, with only about `38-44%` GPU utilization.
  `--sr24-preset mlpall_fixedprefix2_graphsafe` disables the direct cuSPARSELt
  custom path while keeping mixed steps graph-eligible. It is a negative
  diagnostic, not the speed path: at
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_fixedprefix2_graphsafe_speconly_bs16_32_max512_20260630/`
  bs16 still crashed, and bs32 finished at total/full-batch
  `2573.852/2733.996` tokens/s versus dense `2704.280/3460.815`.
  Therefore the next implementation should not rely on eager mixed-step
  fallback or more scalar thresholds. It should implement a graph-stable
  fixed-capacity route table and fused/grouped MLP data format: sparse-only
  rows and dense-important rows use disjoint row sets, row buckets are
  fixed-capacity rather than dynamically allocated through Python
  `nonzero/index_select/index_copy_`, and small important-row sets should be
  coalesced or run concurrently with the 2:4 branch only when the packed tiles
  are large enough to occupy the GPU.
- 2026-06-30 follow-up for the current systems target: the direct row-routed
  fixed-prefix2 serving probe is stable but still not fast enough. In
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_fixedprefix2_rowrouted_probe_20260630/`,
  Llama-3.1/math/K8/max512/bs64 reports dense total/full-batch
  `3361.636/4015.661` tokens/s versus SR24 `3163.212/3842.321`, with healthy GPU
  utilization (`96.7%`) and CUDA Graph profile `{"FULL":49}`. Acceptance is not
  the limiter (`2.478` accepted draft tokens/step for SR24 versus `2.414` dense).
  This confirms the disjoint-row semantics are necessary but the current
  Python/Torch row-list MLP split is still too expensive; the missing piece is a
  fused/grouped route-table operator, not another controller threshold.
- The diagnostic run
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_fixedprefix2_rowrouted_breakdown_20260630/`
  should be used only for coarse graph/util context. It finished with
  `profile:{"FULL":49}` and SR24 GPU util about `88.6%`, but the SR24 Linear
  breakdown events did not flush because the interval was set too high for the
  short run. Do not cite per-linear timing from this root.
- Do not treat graph-capable all-MLP direct cuSPARSELt as a general serving path
  yet. Probes that disabled mixed-step eager fallback and enabled nonuniform
  dense fallback failed at server startup from KV-cache/graph memory pressure,
  not from the sparse kernel itself:
  `sr24_fixedprefix2_directcslt_nonuniformdense_20260630/`,
  `sr24_fixedprefix2_directcslt_nonuniformdense_len2048_20260630/`, and
  `sr24_fixedprefix2_directcslt_nonuniformdense_len1800_20260630/`. The observed
  storage/graph footprint leaves too little KV cache on the 32GB RTX 5090 for
  normal bs64 long-output serving. Any next implementation must reduce
  residual-weight/graph memory, for example by limiting leaves/layers or by a
  faster compressed-residual/fused kernel instead of materializing full dense
  residual rows for all MLP leaves.
- Current optimization conclusion for the user-requested PPoPP-style direction:
  use the 8pp accuracy budget and larger GSM8K gates, but first replace the data
  path. Important rows should be dense-only, unimportant rows 2:4-only, both
  represented by graph-stable fixed-capacity route tables. Route metadata should
  stay on GPU; avoid per-step Python `nonzero`, dynamic `index_select`, and small
  residual GEMMs. If important rows are too few, coalesce them across
  requests/layers or use an explicit dense fallback; only try dense/sparse stream
  overlap after the branches are large enough, because the existing two-stream
  row-routed overlap ablation is negative.
- 2026-06-30 route-table implementation note: `vllm/vllm/speclink_sr24.py` now
  has a narrow vectorized fastpath for `fixed_prefix + bonus` route rows. It
  builds the common uniform-decode prefix/bonus residual rows from device-side
  arange plus a small copied start-row vector, then uses the Triton bucket
  complement helper for base rows. It falls back to the old Python-list path for
  nonuniform valid lengths, missing bonus rows, or non-bonus policies. Syntax
  check:
  `conda run -n spec python -m py_compile vllm/vllm/speclink_sr24.py`.
- The fastpath is correct/useful as a route-table building block, but it is not
  the throughput fix. The hit-validation run
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_fixedprefix2_bonus_vector_route_len1024_smoke_20260630/`
  used `lossy_prefix2_rowrouted_mlp`, bs64/K8/math/max128 with
  `--max-model-len 1024`, and reported
  `scheduler_fixed_prefix_route_vectorized_builds=542`. Breakdown still shows
  the row-routed sparse-base MLP branch dominating:
  `row_routed_mlp_base_gate_up_cuda_ms=1.086 ms/call` and
  `row_routed_mlp_base_down_sparse_cuda_ms=1.089 ms/call`, versus dense branch
  `0.155/0.085 ms/call`. Scheduler route-table construction is measurable
  (`scheduler_fixed_prefix_route_rows_cpu_ms=1.872 ms/step` in this diagnostic),
  but it is not first-order.
- The matching clean short comparison at
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_fixedprefix2_bonus_vector_route_len1024_clean_20260630/`
  is negative: dense EAGLE3 total/full-batch `1599.612/2579.735` tokens/s,
  while the vectorized-route `lossy_prefix2_rowrouted_mlp` path reaches only
  `612.851/692.762`. The same preset at the default `max_model_len=4096` failed
  before serving because all-MLP `dense_rows` storage left only `0.16 GiB`
  available KV cache while `0.52 GiB` was required:
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_fixedprefix2_bonus_vector_route_smoke_20260630/`.
  Therefore next SR24 work should not spend more time on route-row construction
  alone. It must reduce sparse-base MLP cost and memory footprint, e.g. a fused
  grouped MLP/kernel that keeps disjoint dense/sparse rows but avoids inefficient
  small sparse GEMMs and full dense residual storage.
- 2026-06-30 layer-scoped noverify dense follow-up: `vllm/vllm/speclink_sr24.py`
  now supports
  `SPECLINK_SR24_SELECTIVE_DENSE_NONVERIFY_LAYER_IDS_BY_LEAF`, exposed in the
  throughput and lm-eval runners as
  `--sr24-selective-dense-nonverify-layer-ids-by-leaf`. Empty scope preserves
  the old behavior. A scope such as `gate_up_proj=0-7;down_proj=0-7` keeps
  no-mask/noverify MLPs dense only in those layers; other noverify rows use the
  2:4 sparse base, so unimportant rows are not computed sparse and then dense
  again. New lossy-sweep candidates:
  `lossy_prefix2_rowrouted_mlp_front8_dense_noverify_compile` and
  `lossy_prefix2_rowrouted_mlp_front8_dense_noverify_compile_minbase0`.
- The small GSM8K-50 quality gates for this scope are quality-safe under the
  current 8pp budget: front24 `0.7400 -> 0.7200`, front16 `0.7400 -> 0.7400`,
  front8+compile `0.7400 -> 0.7400`, and front8+compile+minbase0
  `0.7400 -> 0.7400`. Relevant roots:
...
- Throughput remains the blocker. Compile-off front24/front16 noverify-scope
  probes reach only bs32/bs64 total speedups `0.835x/0.965x` and
  `0.837x/0.945x`. Enabling `--sr24-default-vllm-compile` recovers front16 to
  about `0.997x/0.997x`, but front8 and front8+minbase0 are still only about
  parity: `0.997x/0.997x` and `0.992x/1.000x`. Do not claim speedup from these
  scope changes; their value is proving a quality-safe controller boundary and
  that vLLM default compile is mandatory for SR24 serving.
- The diagnostic front16 breakdown is at
  ...
  It confirms the scope works: noverify dense MLP calls are only in layers 0-15,
  while layers 16-31 use base sparse Linear calls. The sync-heavy linear timing
  localizes the cost to sparse base MLP calls: about `1.172 ms/call` eager for
  base sparse, while first-half dense noverify MLP is about `0.645 ms/call`
  gate/up and `0.343 ms/call` down. The sparse backend probe at
  ...
  shows why: PyTorch semi-structured sparse eager is slow, CUDA Graph replay can
  make base-only sparse linears faster, but exact base+residual paths still need
  a fused packed kernel to beat dense by 1.2x.
- Current next step: do not continue threshold-only, minbase-only, or
  layer-scope-only sweeps. Build the PPoPP-style data path instead:
  graph-stable fixed-capacity route tables, GPU-resident route metadata,
  disjoint dense-important and 2:4 sparse-unimportant rows, no duplicate
  sparse+dense work per row, and a fused/grouped mixed MLP operator. If
  important rows are too few, coalesce across requests/layers or dense-fallback;
  only use dense/sparse stream overlap after both branches have enough rows to
  occupy the GPU.
- 2026-06-30 noverify-sparse boundary update: the special value
  `--sr24-selective-dense-nonverify-layer-ids-by-leaf none` is now supported.
  It means no no-mask/noverify MLP layer keeps the dense fastpath; those rows
  use only the 2:4 sparse base while verifier prefix/bonus rows keep the normal
  selective dense protection. The first attempt failed at server init because
  stats parsing still treated `none` as a `leaf=ids` string; this was fixed in
  `vllm/vllm/speclink_sr24.py`.
- The clean boundary candidate is
  `lossy_prefix2_rowrouted_mlp_noverify_sparse_compile`. GSM8K-50 quality is
  still safe under the 8pp budget: dense `0.7400`, SR24 `0.7400`, `0pp` delta.
  Short-output math/max128 reaches only bs32/bs64 total speedups
  `0.985x/1.012x`, but math/max512 reaches bs8/16/32/64 total speedups
  `0.960x/0.992x/1.000x/1.239x`. Roots:
  ...
  This is the first quality-safe live SR24 config in this pass that exceeds
  `1.2x`, but only for bs64/max512/math. It does not meet the goal for most
  batch sizes, so the next work remains branch fill and fused/grouped operator
  design, not another noverify scope sweep.
- 2026-06-30 follow-up boundary checks: raising noverify-sparse compile to
  K=12 keeps GSM8K-50 quality safe (`0.7400 -> 0.7400`) but gives only
  bs16/32/64 math/max512 total speedups `1.101x/1.062x/0.978x` and full-batch
  speedups `1.008x/1.014x/0.979x`. Root:
  ...
  Dense EAGLE3 K12 accepts only about `2.43-2.59` draft tokens/step on
  `math_reasoning`, so bigger K mostly increases verifier work instead of
  filling useful accepted-token work.
- Added `lossy_prefix2_rowrouted_mlp_noverify_sparse_overlap_compile` as an
  explicit overlap candidate: `scope=none`, default vLLM compile,
  `route_min_base_rows=128`, and `route_overlap_streams=1`. It validates that
  the current fixed-block overlap path is active, but it is still slower than
  dense: bs16/32/64 math/max512 total speedups `0.999x/0.984x/0.950x` and
  full-batch speedups `0.982x/0.998x/0.948x`; GSM8K-50 remains `0pp` delta.
  Root:
  ...
  Do not repeat K-only or stream-flag-only sweeps as the next optimization.
  The next useful work is a graph-stable fixed-capacity route table plus a
  fused/grouped mixed MLP operator that keeps dense-important and
  2:4-sparse-unimportant rows disjoint without auxiliary Python/PyTorch split
  overhead.
- 2026-06-30 Triton fixed-block assemble ablation:
  `lossy_prefix2_rowrouted_mlp_noverify_sparse_tritonassemble_compile` and
  `--sr24-triton-route-assembly` add an opt-in Triton output-assembly kernel for
  the common fixed prefix/sparse-middle/bonus layout. This path is active and
  quality-safe in the smoke, but it is not a speed fix. The focused root
  ...
  keeps GSM8K-50 at `0.7400 -> 0.7400`, reaches only about `1.009x` total at
  bs32, and the completed raw bs64 stream records give about `0.960x` total
  versus dense. The generated median table missed that bs64 SR24 row despite
  `64/64` successful stream records, so use the raw stream record check for the
  negative read. Do not enable Triton assemble by default; it removes small
  slice copies but does not reduce the dominant sparse-base MLP cost.
- 2026-06-30 low-row sparse branch check:
  `lossy_prefix2_rowrouted_mlp_noverify_sparse_compile_minbase0` sets
  `--sr24-route-min-base-rows 0` while keeping noverify rows sparse-only and the
  verifier prefix/bonus protected. It is quality-safe on GSM8K-50
  (`0.7400 -> 0.7400`), but it is not the low-batch solution. On
  Llama-3.1/math/K8/max512, total/full-batch speedups are bs8
  `1.016x/1.012x`, bs16 `0.954x/1.011x`, bs32 `1.036x/0.997x`, and bs64
  `0.741x/0.946x`. Root:
  ...
  Keep the current `route_min_base_rows=128` guard for the split operator. The
  path needs row coalescing or a fused/grouped sparse+dense MLP kernel, not
  forced tiny sparse branches.
- 2026-06-30 actual K8 row-shape lower bound: for K=8, verifier MLP rows are
  roughly `batch_size*(K+1)` and the fixed prefix2+bonus dense group is
  `3*batch_size`. Microbench roots:
  ...
  The lower-bound result is decisive: full sparse MLP is slower than dense for
  bs8/16-shaped rows (`0.2935ms` vs dense `0.2684ms`, and `0.3554ms` vs dense
  `0.3228ms`), while exact-down row routing is much slower (`1.96x` and
  `1.67x` dense time). This explains why token/layer threshold sweeps cannot
  reach 1.2x at low batch with the current PyTorch/cuSPARSELt split operator.
- `profile_speclink_sr24_cslt_algos.py` sweeps cuSPARSELt `alg_id`. Root:
  ...
  Only alg0 and alg1 are valid on the current stack. alg1 improves rows=72 to
  `1.152x` sparse-vs-dense but gives only `1.029x` at rows=144 and hurts
  rows>=288, where alg0 is best (`1.240x` and `1.553x`). An opt-in live switch
  exists for diagnostics:
  `--sr24-cslt-small-m-alg-id-enable --sr24-cslt-small-m-threshold 96
  --sr24-cslt-small-m-alg-id 1`, mapped to
  `SPECLINK_SR24_CSLT_SMALL_M_ALG_ID_ENABLE`,
  `SPECLINK_SR24_CSLT_SMALL_M_THRESHOLD`, and
  `SPECLINK_SR24_CSLT_SMALL_M_ALG_ID`. Do not enable it by default.
- `lossy_prefix2_rowrouted_mlp_noverify_sparse_compile_smallm_alg1` is the
  noverify-sparse compile boundary plus the small-M alg1 switch. Root:
  ...
  GSM8K-50 remains unchanged (`0.7400 -> 0.7400`), but math/K8/max512 reaches
  only bs8/16/32/64 total speedups `1.015x/1.002x/0.990x/0.963x` and full-batch
  speedups `0.993x/1.008x/0.997x/0.947x`. It is a useful operator-selection
  diagnostic, not the 1.2x path.
- K inflation is also not the fix for this path. The K16 throughput-only probe
  at
  ...
  gives bs8/16/32/64 total speedups `1.010x/0.989x/0.926x/0.968x`; the earlier
  K12 noverify-sparse root reaches at best `1.101x` total at bs16 but only about
  `1.008x` full-batch. Do not repeat K-only sweeps unless the operator/data path
  changes. The next viable route is a real small-M 2:4 kernel or a graph-stable
  grouped/fused MLP layout that coalesces sparse rows without increasing
  verifier work.
- 2026-06-30 scalar Triton base-2:4 prototype result: the standalone script
  `examples/evaluate/eval-guidellm/scripts/profile_speclink_sr24_triton_base24_mlp.py`
  validates a GPU-resident `values + k0/k1` compressed data layout, but the
  gather-multiply kernel is far too slow because it does not use sparse tensor
  cores. Root:
  ...
  At actual K8 row shapes, Triton base24 reaches only `0.033x/0.021x/0.017x/
  0.016x` dense speed for rows `72/144/288/576`. Do not try to integrate this
  scalar kernel into serving. Any custom sparse base operator must be
  tensor-core-backed through cuSPARSELt/CUTLASS or a CUDA kernel with an
  equivalent compressed sparse layout.
- 2026-06-30 fixed-block Triton assembly result:
  `lossy_prefix2_rowrouted_mlp_noverify_sparse_tritonassemble_compile` is
  quality-safe on GSM8K-50 (`0.7400 -> 0.7400`) and confirms that unimportant
  verifier/noverify rows are sparse-only without dense overwrite, but it only
  reaches math/K8/max128 bs8/16/32/64 total speedups
  `0.996x/1.000x/1.005x/1.002x` and full-batch speedups
  `0.996x/1.002x/1.000x/0.989x`. Root:
  ...
  Output assembly is no longer the main limiter. The remaining bottleneck is
  small-M tensor-core sparse MLP efficiency and split-branch launch structure.
  Future work should focus on graph-stable fixed-capacity route descriptors and
  a fused/grouped dense-important plus 2:4-sparse-unimportant MLP operator.
- 2026-06-30 gate_up-only noverify-sparse probe:
  `lossy_prefix2_gateup_only_noverify_sparse_compile` restricts SR24 to
  `gate_up_proj`, keeps `down_proj` dense, and leaves unimportant/noverify
  gate_up rows sparse-only. Root:
  ...
  It is quality-safe on GSM8K-50 (`0.7400 -> 0.7400`) but still only reaches
  math/K8/max128 bs8/16/32/64 total speedups `0.997x/1.006x/1.021x/1.001x`
  and full-batch speedups `0.997x/1.001x/0.996x/1.005x`. Removing the sparse
  down branch is therefore not enough; the gate/up small-M sparse branch and
  row-routing/launch structure remain the limiter.
- 2026-06-30 channel-pair gate/up split probe:
  `gateup_channel_pair_dense25_eager` and
  `gateup_channel_pair_dense50_eager` split the gate/up intermediate channels
  instead of verifier rows: the highest-norm 25% or 50% channels stay dense,
  the rest use a full-row 2:4 sparse gate/up branch, and `down_proj` stays
  dense. The default-compile attempt hit a Torch compile/lazy-storage failure
  while tracing the grouped down weight, so the quality gate was rerun eager at
  ...
  GSM8K-50 failed badly: dense25 `0.7200 -> 0.5000` (`-22pp`) and dense50
  `0.7200 -> 0.5600` (`-16pp`). Do not continue channel-splitting across all
  gate/up layers unless a new sensitivity model scopes it much more narrowly.
- 2026-06-30 minbase/prefix boundary probes after the bs64 gate-up breakdown:
  `lossy_prefix2_rowrouted_mlp_noverify_sparse_compile_minbase256` is
  quality-safe on GSM8K-50 (`0.7400 -> 0.7400`) but only reaches bs64
  math/K8/max128 full/total speedups `1.0005x/0.9892x`; root
  ...
  `lossy_prefix2_gateup_only_noverify_sparse_minbase256_compile` is also
  quality-safe and reaches only `0.9934x/1.0060x`; root
  ...
  Prefix1 variants protect only draft position 0 plus the verifier bonus row:
  `lossy_prefix1_gateup_only_noverify_sparse_minbase128_compile` is quality
  safe and gives bs64 max128 `0.9990x/1.0115x`, while
  `lossy_prefix1_mlp_noverify_sparse_minbase128_compile` gives
  `0.9914x/0.9982x`; root
  ...
  A longer-output bs64/max512 sanity for the gate_up-only prefix1 candidate is
  still parity, `0.9992x/1.0004x`; root
...
- 2026-06-30 latest bottleneck read:
  ...
  isolates the current gate_up-only noverify-sparse path. Clean serving is
  dense/SR24 full-batch `2768.973/2784.583` tok/s and total
  `2377.825/2325.595` tok/s, so the path is roughly parity. SR24 GPU util is
  high (`92.1%`) and graph coverage is reasonable (`FULL=96,NONE=32`), while
  diagnostic timing shows the row-routed gate/up branch at
  `1.131 ms/call`, with base sparse `0.936 ms/call`, dense GEMM
  `0.164 ms/call`, and gather/scatter about `0.031 ms/call`. Average rows per
  call are only about `257 total / 171 base / 86 dense`. The current blocker is
  therefore small-M tensor-core sparse MLP efficiency plus split branch launch
  structure, not output length, final assembly, or duplicate dense recompute.
  Future SR24 optimization should move to fixed-capacity route descriptors and
  a grouped/fused dense-important plus 2:4-sparse-unimportant MLP operator.
- 2026-07-01 fixed-capacity fill ablation:
  `examples/evaluate/eval-guidellm/scripts/profile_speclink_sr24_packed_verifier_mlp.py`
  now supports `--capacity-multiple`, `--min-dense-capacity`, and
  `--min-base-capacity`. The new `packed_capacity_padded` variants copy active
  dense/base rows into fixed-capacity branch buffers before the MLP GEMMs, so
  they measure whether padding small branches to graph-stable tensor-core row
  capacities can solve the underfill problem. Results:
  ...
  for capacity multiple 64. Padding is not the fix: with multiple 64,
  padded speedups versus dense are only about `0.49x/0.59x/0.86x` at
  bs8/16/32. bs64 prefix2 reaches `1.199x`, but only because active rows
  already fill the fixed buffers (`192/192` dense and `384/384` sparse).
  The useful signal is that two-stream unpadded packed reaches bs64 prefix2
  `1.241x` standalone, while low-batch remains far below dense. Next work
  should coalesce sparse rows across more useful work or implement a real
  grouped/fused sparse operator; do not add padding-only live serving logic.
- 2026-07-01 useful-row coalescing ablation:
  `profile_speclink_sr24_packed_verifier_mlp.py` also supports
  `--coalesce-factors`. This simulates grouping several independent verifier
  blocks with the same weights into one larger dense/sparse branch, and reports
  both speedup versus one coalesced dense GEMM and versus running the original
  batch-size dense block serially. Root:
  ...
  Key graph-captured K8/prefix2 results: original bs8 needs coalesce 8
  (effective bs64) to reach mixed-vs-coalesced-dense `1.242x`; bs16 needs
  coalesce 4 to reach `1.240x`; bs32 needs coalesce 2 to reach `1.241x`.
  Coalescing dense alone also gives large launch/shape savings, so this is an
  optimistic upper bound, not a live-serving claim. The design implication is
  precise: low batch cannot be fixed by dummy padding, but it can become viable
  if a scheduler/operator can group real useful verifier work to effective
  batch around 64. Without such grouping, use dense fallback for underfilled
  branches.
- 2026-07-01 lossy SR24 optimization rule:
  subsequent SR24 speed work should allow up to 8 percentage points absolute
  accuracy loss instead of requiring lossless behavior. Use GSM8K-CoT
  `limit>=50` as the quick gate before throughput. The latest down_proj
  layer-scope gate shows full down_proj base-only fails (`0.72 -> 0.48`),
  tail24 fails (`0.72 -> 0.56`), tail20 is exactly on the 8pp boundary
  (`0.72 -> 0.64`), and tail16/tail8 pass. Tail20 still does not create a
  steady full-batch speedup: bs32/64 full-batch speedups are about
  `0.844x/0.996x`, despite accepted draft length increasing. A gated
  `row_routed_down` CUDA-stream overlap probe also stays at parity
  (`0.990x/0.997x` full-batch for bs32/64). Read this as a systems result:
  accepted length and duplicate dense correction are not the current limiter;
  the next credible path is fixed-capacity route descriptors plus grouping of
  real useful rows to effective batch around 64, then a graph-stable
  grouped/fused dense-important plus 2:4-sparse-unimportant operator. Do not
  spend more time on lossless gates, K-only sweeps, or threshold-only tuning
  until the data-format/operator path changes.
- 2026-07-01 uniform-attach tail20 check:
  trying to restore CUDA Graph coverage by attaching all `down_proj` layers to
  SR24 and using dense verify fallback for layers 0-11 did not work. Root:
  ...
  The candidate still recorded only `{"NONE": ...}` graph modes and full-batch
  speedups were about `0.838x/0.996x` for bs32/64. Treat this as evidence that
  "uniform attach + per-layer Python fallback" is not enough. A graph-stable
  path needs one uniform operator branch shape and fixed route descriptors, not
  per-layer module attributes selecting dense versus sparse implementations.
- 2026-07-01 relaxed-prefix 8pp check:
  `lossy_prefix1_gateup_only_noverify_sparse_minbase128_compile` protects only
  draft position 0 plus the verifier bonus row with dense `gate_up_proj`; other
  draft/no-verify gate/up rows stay 2:4 sparse-only, and split routing requires
  at least 128 base rows. Root:
  ...
  It passes GSM8K-50 with no drop (`0.7400 -> 0.7400`), but math/K8/max512
  bs8/16/32/64 full-batch speedups are only
  `0.997x/0.967x/0.997x/1.003x`; total speedups are
  `1.035x/1.041x/0.975x/0.776x`. This closes the easy "allow 8pp loss and
  protect fewer important tokens" route: the current split operator remains
  steady-state parity. Do not continue with scalar prefix/threshold sweeps
  unless the data format or operator launch structure changes first.
- 2026-07-01 fixed-capacity grouped bucket planner refresh:
  `/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_packed_grouped_bucket_k8_bs8_64_prefix12_20260701/operator_planner.md`
  now covers bs `8/16/32/64`, K=8, prefix `1/2`, capacity multiple 64, and
  coalesce factors `1/2/4/8`. Use the `mixed local speedup` columns for the
  operator decision; `mixed serial speedup` is only an optimistic scheduler
  upper bound. For prefix2, the mixed operator reaches the local `1.2x` target
  only when the effective batch is about 64: bs8 needs coalesce 8, bs16 needs
  coalesce 4, bs32 needs coalesce 2, and bs64 is ready with coalesce 1. This is
  the clearest current systems direction: design a fixed-capacity route
  descriptor and scheduler grouping path that fills real useful rows to about
  effective bs64, otherwise fall back to dense. Do not use dense-coalescing or
  serial-upper-bound rows as SR24 serving claims.
- 2026-07-01 current-tree noverify-sparse correction:
  do not use `lossy_prefix2_rowrouted_mlp_noverify_sparse_compile` as a
  quality-safe candidate. A fresh GSM8K-CoT `limit=50`, max-new-tokens 512 run
  on the current tree at
  ...
  gives dense `0.7800` and SR24 `0.1400` (`-64pp`), with 35 paired
  regressions and clipped long outputs. Older same-name roots that reported
  no loss should be treated as stale or not exercising the current sparse-only
  noverify path. The useful rule remains: important verifier rows should be
  dense-only and unimportant rows sparse-only, but broad noverify MLP
  sparsification is too destructive. Use layer/leaf-scoped candidates only as
  diagnostics, and prioritize the grouped fixed-capacity mixed MLP operator
  plus dense fallback for underfilled branches.
- 2026-07-01 grouped scheduler policy artifact:
  `examples/evaluate/eval-guidellm/scripts/derive_sr24_packed_mlp_planner.py`
  now writes `scheduler_policy.json` in addition to the existing CSV/Markdown
  planner outputs. Current refreshed policy:
  ...
  It is intentionally an operator-local policy, not a serving claim. For
  K=8/prefix2 it says bs8/16/32 must use dense fallback unless the scheduler can
  group 8/4/2 ready verifier blocks respectively, reaching effective bs64; bs64
  can use the mixed operator directly. Future live work should consume this JSON
  to drive a verifier-MLP grouping queue keyed by module weights and fixed route
  descriptor.
- 2026-07-01 live scheduler-policy gate:
  `vllm/vllm/speclink_sr24.py` can now opt in to the planner artifact with
  `SPECLINK_SR24_SCHEDULER_POLICY_PATH=/path/to/scheduler_policy.json`. The
  GuideLLM matrix runner and lm-eval runner expose the same setting as
  `--sr24-scheduler-policy-path`. The current implementation is a conservative
  gate only: if the policy says the fixed-block mixed MLP is underfilled, the
  block falls back to dense and records
  `row_routed_mlp_fixed_block_scheduler_policy_dense_fallback` in the SR24
  breakdown. A bs8/K8/prefix2 smoke at
  ...
  confirmed 384 fixed-block hits and 384 policy dense fallbacks. This is not
  the final grouped-queue implementation; the next useful work is still to
  actually coalesce ready verifier MLP blocks to effective bs64 before launching
  the dense-important plus 2:4-sparse-unimportant operator.
  Follow-up on the same day tightened this gate: planner rows that require the
  offline `packed_parallel` mixed operator now fall back to dense unless
  `SPECLINK_SR24_SCHEDULER_POLICY_ALLOW_LEGACY_MIXED=1` is explicitly set. This
  prevents applying the microbench planner to the current live legacy split
  MLP. A bs64/K8/prefix2 smoke at
  ...
  recorded 41 mixed mask states, 1280 fixed-block descriptor hits, and 1280
  `row_routed_mlp_fixed_block_scheduler_policy_operator_unimplemented` dense
  fallbacks. Treat this as correctness/claim hygiene, not a speedup.
- 2026-07-01 live packed-parallel overlap probe:
  the planner's `mixed_operator=packed_parallel` is now recognized by the live
  policy gate only when `SPECLINK_SR24_ROUTE_OVERLAP_STREAMS=1`; fixed-prefix
  overlap-stream plans can try CUDA Graph replay only with the narrower
  `SPECLINK_SR24_ROUTE_OVERLAP_ALLOW_CUDAGRAPH=1` /
  `--sr24-route-overlap-allow-cudagraph` opt-in. A dispatch smoke at
  ...
  showed 768 mixed-allowed fixed-block overlap calls and 224 underfilled dense
  fallbacks. A no-breakdown graph smoke at
  ...
  succeeded and showed some FULL graph replay, but a more meaningful bs64
  speed sanity at
  ...
  was negative: SR24 was `0.836x` total and `0.943x` full-batch vs dense, with
  essentially identical accepted draft tokens/step and lower GPU utilization.
  This rules out the current Python-level stream-overlap fixed-block path as
  the final `1.2x` solution; the next useful implementation must be a lower
  overhead grouped/fused operator or a better small-M sparse kernel.
- 2026-07-01 follow-up live speed ablations:
  disabling the no-verify dense MLP wrapper made the same bs64/max128 sanity
  worse (`0.808x` total, `0.907x` full-batch), so the wrapper is not the
  slowdown source. Enabling Triton fixed-block assembly also made it worse
  (`0.803x` total, `0.910x` full-batch), so output assembly is not the dominant
  limiter. Rechecking the existing down-proj-only candidate on the same
  current-tree bs64/max128 setup was also negative (`0.788x` total,
  `0.902x` full-batch) despite higher accepted draft length; GPU util dropped
  to about 63%. Relevant roots:
  ...
  These make the bottleneck sharper: accepted length is not the issue; the
  current live split sparse execution underutilizes the GPU. Continue with a
  true grouped/fused sparse operator or small-M sparse kernel, not wrapper,
  assembly, or down-only policy tweaks.
- 2026-07-01 leaf-aware cuSPARSELt alg-pair refresh:
  the CUTLASS semi-structured backend is not usable on the current RTX 5090
  stack; the probe at
  ...
  failed with `sparse_semi_structured_mad_op : Supported only on GPUs with
  compute capability 8.x`. The valid path is still cuSPARSELt. A projection
  alg-pair sweep at
  ...
  showed the best pairs are rows48 `gate_up=1/down=1`, rows96
  `gate_up=0/down=1`, rows192 `gate_up=0/down=1`, and rows384
  `gate_up=0/down=0`. The SR24 runtime now supports
  `SPECLINK_SR24_CSLT_SMALL_M_THRESHOLD_BY_LEAF` and
  `SPECLINK_SR24_CSLT_SMALL_M_ALG_ID_BY_LEAF`, and the GuideLLM/lm-eval
  runners expose the matching `--sr24-cslt-small-m-*-by-leaf` flags.
  The current clean candidate is
  `lossy_prefix2_verifier_only_outputbuf_leafalg_pair_compile`, using global
  alg1 for rows<=64 and down_proj alg1 for rows<=256. It passes the GSM8K-50
  8pp gate (`0.7800 -> 0.7400`, -4pp), but math_reasoning max128 throughput is
  still not a speedup:
  ...
  Total speedups are bs32 `0.990x` and bs64 `0.975x`; full-batch speedups are
  bs32 `1.014x` and bs64 `0.991x`.
- 2026-07-01 underfilled sparse-branch diagnosis:
  an eager linear breakdown at
  ...
  showed why the leaf-aware alg knob did not move end-to-end speed. With the
  quality-safe prefix2 route, `route_min_base_rows=384` causes most live decode
  steps to fall back to dense (`row_routed_mlp_full_dense_fallback_small_base`
  count 2432; only about 136 base rows/step in the probe). Lowering the guard
  makes sparse run more often, but hurts serving: minbase96 gives total
  speedups bs32 `0.734x`, bs64 `0.766x`, and prefix1/minbase128 gives bs32
  `0.715x`, bs64 `0.967x`.
  Python stream overlap is also not viable here:
  ...
  gives bs64 total `0.672x` versus the dense row from the same earlier gate and
  low GPU utilization. Treat the current PyTorch/cuSPARSELt split MLP as an
  underfilled-small-kernel prototype. The next credible path to 1.2x is a real
  grouped/fused fixed-route operator or a faster small-M 2:4 kernel, with dense
  fallback when useful sparse rows cannot fill the operator.
- 2026-07-01 verifier-only prefix boundary and fallback fastpath:
  `vllm/vllm/speclink_sr24.py` now short-circuits fixed-block row-routed MLP
  underfill: when `base_count < route_min_base_rows`, it directly uses the
  MLP-level dense fallback (`reason=fixed_block_small_base`) instead of
  returning to the generic row-routed path and rebuilding route state before
  falling back. A small math_reasoning bs32/64 probe at
  ...
  shows this only closes minor fallback overhead: bs32 total/full
  `1.011x/1.010x`, bs64 total/full `0.997x/0.962x`.
  `scripts/run_sr24_lossy_speed_quality_sweep.py` now has verifier-only
  prefix1/prefix0 candidates:
  `lossy_prefix1_verifier_only_outputbuf_leafalg_pair_compile` and
  `lossy_prefix0_verifier_only_outputbuf_leafalg_pair_compile`. These keep
  ordinary no-mask decode dense and only change the speculative verifier block.
  GSM8K-50 quality passes the 8pp gate, but speed does not:
  ...
  reports prefix1 `0.7800 -> 0.7600` (-2pp), best total/full
  `0.858x/0.999x`, and prefix0 `0.7800 -> 0.7400` (-4pp), best total/full
  `0.752x/0.930x`. This separates the earlier noverify-sparse failure from
  verifier-only prefix relaxation: prefix0/1 are quality-feasible, but the
  current sparse branch is too inefficient to speed up. A prefix1
  `--sr24-route-overlap-streams` probe at
  ...
  gives bs32 total/full `0.695x/0.858x` and bs64 total/full `0.992x/0.991x`;
  Python-level dense/sparse stream overlap is therefore not the main path.
  The next systems implementation should be a fixed-route grouped/fused sparse
  MLP operator or a faster small-M 2:4 kernel, with dense fallback whenever
  useful sparse rows cannot fill the operator.
- 2026-07-01 all-corrected compressed residual check:
  `all_corrected_24` with `compressed_dense@cuda` is GPU-resident, but it is
  not a speed path. The bs64/math/max64 eager breakdown at
  ...
  reports dense full-batch `2796.692` tok/s versus all-corrected `862.656`
  tok/s. The hot cost is repeated residual materialization for `gate_up_proj`
  chunks: `compressed_residual_materialize_cuda_ms=3991.194`,
  `compressed_residual_gemm_cuda_ms=1537.202`, and
  `base_sparse_linear_cuda_ms=1319.383`. Full residual-weight caching removes
  most materialization calls, but for `max_model_len=4096` it failed KV-cache
  sizing after profile-run. A diagnostic `max_model_len=2048` run at
  ...
  reached only `1659.350` all-corrected full-batch tok/s versus dense
  `2609.329` tok/s. Do not auto-enable full `gate_up_proj` residual caching
  for normal 4096-token serving. Treat exact all-corrected compressed residual
  as a control/debug path, not the lossy acceleration path.
- 2026-07-01 SR24 breakdown runner hygiene:
  `--sr24-breakdown` now forces eager/no-CUDA-graph in both GuideLLM and
  lm-eval runners. Breakdown mode uses CUDA events plus Python counters guarded
  by locks; allowing Dynamo/Inductor to trace this diagnostic code can fail
  during vLLM startup profiling.
- 2026-07-01 continuation, lossy-system boundary:
  `--sr24-base-only-allow-compile` in the GuideLLM matrix runner now implies
  `--sr24-allow-cudagraph`, because otherwise the runner still added
  `--enforce-eager` and the flag had no serving effect. With that fixed,
  MLP-only `base_only_24` is the current speed upper bound, not a quality-safe
  method: bs64/math/max256 with `target_leafs=gate_up_proj,down_proj` and no
  residual correction reached dense total/full `2325.671/3444.110` tok/s
  versus base-only `3905.823/6351.732` tok/s, CUDA graph
  `{"FULL": 96, "NONE": 2}`, accepted draft tokens/step `3.3445`, and avg GPU
  util `88.1%` at
  ...
  The same MLP-only sparse-all-token policy fails the relaxed quality target:
  GSM8K-50 drops from `0.7200` dense to `0.2200` base-only (-50pp) at
  ...
  Two noverify-sparse policies also fail GSM8K-50 even with prefix2 dense
  verifier protection: all-MLP noverify-sparse `0.7800 -> 0.1200` (-66pp) and
  gate_up-only noverify-sparse `0.7800 -> 0.4200` (-36pp). The quality-safe
  boundary remains verifier-only routing: prefix2 fixed-block/output-buffer
  verifier-only passes `0.7800 -> 0.7400` (-4pp), but bs64/math/max128 is not
  faster than dense (`2225.172` vs `2243.093` total tok/s; `3021.742` vs
  `3035.591` full-batch tok/s) at
  ...
  Current direction: do not sparse ordinary/no-mask decode rows for reasoning
  tasks. Keep ordinary rows dense; only route verifier draft rows by risk, and
  implement the speed path as a grouped/fused fixed-route MLP operator or a
  better small-M 2:4 sparse kernel. Python/PyTorch split dense+sparse branches
  and post-sparse dense correction are not enough for the 1.2x target.
- 2026-07-01 late 8pp tail-sparse probe:
  `scripts/run_sr24_lossy_speed_quality_sweep.py` now includes fine-grained
  tail noverify-sparse candidates:
  ...
  `lossy_prefix1_rowrouted_mlp_front30_dense_noverify_compile`, and
  `lossy_prefix0_rowrouted_mlp_front30_dense_noverify_compile`. These keep
  ordinary/no-verify MLP rows dense in early layers and make only the last
  2-4 layers sparse-only; verifier draft rows still use the fixed-prefix
  dense-important plus sparse-unimportant split. The script quality gate also
  has a small epsilon so exactly -8pp is not rejected by floating error.
  GSM8K-50 quality results at
  ...
  show front28/prefix2 `0.7800 -> 0.7400` (-4pp) and front28/prefix1
  `0.7800 -> 0.7400` (-4pp) pass. Front30/prefix2 was exactly on the
  8pp boundary (`0.7800 -> 0.7000`, recorded as fail before the epsilon fix),
  and front30/prefix1 failed (`0.7800 -> 0.6600`, -12pp). A short
  math_reasoning throughput probe at
  ...
  does not meet the 1.2x target: front28/prefix2 speedups are bs32
  total/full `0.976x/1.032x` and bs64 `1.127x/0.984x`; front28/prefix1
  speedups are bs32 `1.046x/1.015x` and bs64 `1.025x/0.962x`.
  This confirms the controller can spend an 8pp budget on more sparse rows,
  but current PyTorch/cuSPARSELt fixed-block split MLP still lacks stable
  full-batch speedup. Do not keep sweeping prefix/layer thresholds as the main
  path. Next work should be the PPoPP-style data-format/operator path:
  fixed route descriptors, reusable branch workspaces, dense fallback for
  underfilled sparse rows, and a fused/packed fixed-block MLP where
  dense-important rows and 2:4 sparse-unimportant rows run with low-overhead
  concurrency.
- 2026-07-02 8pp loss-budget refresh:
  Prefix0 verifier-only remains quality-feasible in tiny gates, but it does not
  produce stable serving speedup with the current split PyTorch/cuSPARSELt
  implementation. The live bs64 prefix0 gate at
  ...
  is only about equal to dense (`2336.7` vs `2326.3` tok/s), and the
  policy/overlap approximation at
  ...
  is slower (`1926.3` vs `2323.3` tok/s). Broad noverify sparse is not
  quality-safe: GSM8K-50 drops from `0.7800` to `0.1400` at
  ...
  The best relaxed-quality tail-sparse point is still prefix1/front28, which
  passed the earlier GSM8K-50 gate (`0.7800 -> 0.7400`, -4pp), but a new
  math_reasoning bs8/16/32/64 run at
  ...
  gives total speedups `0.889x`, `0.820x`, `1.035x`, and `1.032x`. A
  prefix1/front28 stream-overlap bs64 probe at
  ...
  is also slower (`2572.6` vs `2662.1` tok/s). The optimization path should now
  be treated as systems work: fixed-capacity verifier-block data format,
  route descriptors, grouped queueing to effective bs64 for small client
  batches, dense bypass for underfilled groups, and a real fused/packed mixed
  MLP operator. Do not spend more mainline time on Python stream overlap or
  scalar threshold sweeps unless they feed that operator design.
- 2026-07-02 rowguard16 update: `--sr24-selective-dense-nonverify-max-rows`
  is wired through both the throughput matrix runner and `run_lm_eval_accuracy.py`.
  The matrix runner also sets
  `SPECLINK_SR24_SELECTIVE_DENSE_NONVERIFY_STATIC_ROWS` to the current client
  batch size, and lm-eval sets it to `max_num_seqs`, so torch.compile never
  captures a symbolic row-count comparison. Keep this guard for compile-safe
  experiments, but do not treat it as the speed solution: GSM8K-50 stayed
  equal to dense (`0.7800 -> 0.7800`) at
  ...
  while math_reasoning total speedups were bs8 `0.895x`, bs16 `0.860x`,
  bs32 `0.822x`, and bs64 `1.007x` across
  ...
  The next optimization should be a real fixed-route grouped/fused mixed MLP
  operator, not more scalar threshold sweeps.
- 2026-07-02 fixed-block assembly/output-buffer ablation: `_triton_fixed_block_assemble`
  now accepts an optional preallocated output tensor, and both fixed-block MLP
  and fixed-block down-proj routes can use the reusable output workspace when
  `--sr24-triton-route-assembly --sr24-fixed-block-output-buffer` are set. This
  is an explicit ablation only, not a default speed path. The paired candidates
  in `scripts/run_sr24_lossy_speed_quality_sweep.py` are
  `lossy_prefix2_front28_policy_outputbuf_compile` and
  `lossy_prefix2_front28_policy_outputbuf_tritonassemble_compile`. Short bs64
  sanity runs show these small assembly/workspace tweaks do not solve the
  bottleneck:
  ...
  gives total/full speedups `0.638x/0.555x` for the no-Triton output-buffer
  paired candidate at max tokens 64, and
  ...
  gives total speedup `0.527x` when Triton assembly is added under an underfilled
  16-request smoke. The 16-request smoke is intentionally not a bs64 performance
  claim; it only demonstrates underfill sensitivity. The useful conclusion is
  unchanged: allocator-free assembly is not the missing 1.2x mechanism. The
  main path remains a dependency-safe grouped verifier queue plus a fused/packed
  mixed MLP operator; keep the Triton assembly path as a diagnostic switch.
- 2026-07-02 compressed residual Triton refresh:
  `scripts/profile_speclink_sr24_compressed_residual_kernel.py` now emits a
  diagnostic warning when the packed residual-only Triton kernel is slower than
  materialized residual GEMM or has large max diff. The current Llama MLP shape
  refresh is:
  ...
  Best Triton residual-only rows are slower on all tested shapes:
  `triton/dense` is `6.026x` for rows64/down, `3.676x` for rows64/gate_up,
  `7.837x` for rows256/down, and `6.938x` for rows256/gate_up; max abs diff is
  also `0.25-1.0`. Keep `--sr24-compressed-residual-triton` as a diagnostic
  switch only. It should not be used as the `all_corrected_24` speed path.
  The exact all-corrected route needs a fused base+residual operator if it is
  to become useful; residual-only Triton matmul still pays base sparse work and
  is slower than materialized residual GEMM.
