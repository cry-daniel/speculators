...
def sr24_enabled_for_batch(
    ...
    if method not in SR24_METHODS:
        return False
    min_batch = int(getattr(args, "sr24_min_enabled_batch_size", 0) or 0)
    return min_batch <= 0 or int(batch_size) >= min_batch
...
def sr24_has_narrow_residual_scope(args: argparse.Namespace) -> bool:
    ...
    linear module. A leaf-only scope such as `gate_up_proj` is still 32 large
    Llama layers and can OOM while prewarming. The auto fastpath is intended
    for targeted operator ablations, so require an explicit layer scope before
    enabling it.
    """
    scoped_fields = (
...
def sr24_compile_cache_root(
    ...
    fingerprint = {
        ...
        "route_overlap_streams": args.sr24_route_overlap_streams,
        "route_overlap_allow_cudagraph":
        args.sr24_route_overlap_allow_cudagraph,
...
def non_sr24_compile_cache_root(
    args: argparse.Namespace,
    method: str,
    batch_size: int = 0,
) -> Path:
    spec_k = fixed_spec_k_for_method(args, method)
    fingerprint = {
        "cache_version": "non_sr24_matrix_v1",
        "method": method,
        "batch_size": int(batch_size),
        "base_model": str(BASE_MODEL),
        "fastdraft_model": (
            str(FASTDRAFT_MODEL)
            if method in {"vllm_fastdraft", "smurfs_fastdraft"} else ""
        ),
        "eagle3_speculator_model": (
            str(EAGLE3_SPECULATOR_MODEL) if method_uses_eagle3(method) else ""
        ),
        "spec_k": spec_k,
        "fastdraft_k": int(args.fastdraft_k),
        "eagle3_k": int(args.eagle3_k),
        "smurfs_initial_k": int(args.smurfs_initial_k),
    ...
    return EVAL_ROOT / "temp/vllm_compile_cache" / f"non_sr24_{digest}"
...
def build_vllm_command(args: argparse.Namespace, method: str, port: int,
    ...
    sr24_active = sr24_enabled_for_batch(args, method, batch_size)
    compilation_method = method if sr24_active else "dense_baseline"
    compilation_config = effective_vllm_compilation_config(
        args, compilation_method, batch_size)
    ...
    if sr24_active and sr24_requires_enforce_eager(args, method):
...
def start_server(args: argparse.Namespace, method: str, batch_size: int,
    ...
    sr24_active = sr24_enabled_for_batch(args, method, batch_size)
    if method not in SR24_METHODS or not sr24_active:
        # Keep dense/FastDraft/EAGLE3 cache entries separate from SR24 runs.
        # vLLM's default compile cache key does not include our SPECLINK_SR24_*
        # env branches, and stale SR24 graphs can otherwise be replayed by a
        # later dense baseline process.
        dense_cache_root = non_sr24_compile_cache_root(args, method, batch_size)
        server_env["VLLM_CACHE_ROOT"] = str(dense_cache_root)
        command_meta["vllm_compile_cache_root"] = str(dense_cache_root)
    if method in SR24_METHODS and sr24_active:
        ...
        sr24_scheduler_policy_path = (
            str(Path(args.sr24_scheduler_policy_path).resolve())
            if args.sr24_scheduler_policy_path else ""
        )
        command_meta["sr24_effective_enabled"] = True
        command_meta["sr24_batch_fallback"] = False
        ...
        server_env.update({
            ...
            "SPECLINK_SR24_CSLT_SMALL_M_THRESHOLD":
            str(args.sr24_cslt_small_m_threshold),
            "SPECLINK_SR24_CSLT_SMALL_M_ALG_ID":
            str(args.sr24_cslt_small_m_alg_id),
            ...
            "SPECLINK_SR24_ROW_ROUTED_MLP_FIXED_BLOCK_DENSE_FILL":
            "1" if args.sr24_row_routed_mlp_fixed_block_dense_fill else "0",
            "SPECLINK_SR24_FIXED_BLOCK_INPUT_BUFFER":
            "1" if args.sr24_fixed_block_input_buffer else "0",
            "SPECLINK_SR24_FIXED_BLOCK_OUTPUT_BUFFER":
            "1" if args.sr24_fixed_block_output_buffer else "0",
            "SPECLINK_SR24_SCHEDULER_POLICY_PATH":
            sr24_scheduler_policy_path,
            "SPECLINK_SR24_SCHEDULER_POLICY_ALLOW_LEGACY_MIXED":
            "1" if args.sr24_scheduler_policy_allow_legacy_mixed else "0",
            "SPECLINK_SR24_SCHEDULER_POLICY_ALLOW_SINGLE_BLOCK_PACKED_PARALLEL":
            "1" if args.sr24_scheduler_policy_allow_single_block_packed_parallel else "0",
            ...
            "SPECLINK_SR24_SCHEDULER_POLICY_NEAR_FULL_TOLERANCE":
            str(args.sr24_scheduler_policy_near_full_tolerance),
            "SPECLINK_SR24_FIXED_BLOCK_CAPACITY_PADDING":
            "1" if args.sr24_fixed_block_capacity_padding else "0",
            "SPECLINK_SR24_FIXED_BLOCK_CAPACITY_ZERO_DUMMY":
            "1" if args.sr24_fixed_block_capacity_zero_dummy else "0",
            "SPECLINK_SR24_SCHEDULER_POLICY_DENSE_BYPASS":
            "1" if args.sr24_scheduler_policy_dense_bypass else "0",
            "SPECLINK_SR24_GROUPED_QUEUE_SHADOW":
            "1" if args.sr24_grouped_queue_shadow else "0",
            "SPECLINK_SR24_GROUPED_QUEUE_MAX_WAIT_BLOCKS":
            str(args.sr24_grouped_queue_max_wait_blocks),
            "SPECLINK_SR24_GROUPING_TRACE":
            "1" if args.sr24_grouping_trace else "0",
            "SPECLINK_SR24_GROUPING_TRACE_PATH":
            str(run_dir / "speclink_sr24_grouping_trace.jsonl")
            if args.sr24_grouping_trace else "",
            ...
            "SPECLINK_SR24_NOVERIFY_DENSE_MLP_FASTPATH":
            ...
            args.sr24_selective_dense_nonverify_layer_ids_by_leaf,
            "SPECLINK_SR24_SELECTIVE_DENSE_NONVERIFY_MAX_ROWS":
            str(args.sr24_selective_dense_nonverify_max_rows),
            "SPECLINK_SR24_SELECTIVE_DENSE_NONVERIFY_STATIC_ROWS":
            str(batch_size),
        ...
        "SPECLINK_SR24_FIXED_PREFIX_ROUTE_DESCRIPTOR_ONLY":
        "1" if args.sr24_fixed_prefix_route_descriptor_only else "0",
        "SPECLINK_SR24_ROUTE_OVERLAP_STREAMS":
        "1" if args.sr24_route_overlap_streams else "0",
        "SPECLINK_SR24_ROUTE_OVERLAP_ALLOW_CUDAGRAPH":
        "1" if args.sr24_route_overlap_allow_cudagraph else "0",
        ...
        "1" if args.sr24_adaptive_dense_fallback else "0",
            ...
            "SPECLINK_SR24_BUCKET_DENSE_COMPUTE_ACTIVE_ONLY":
            "1" if args.sr24_bucket_dense_compute_active_only else "0",
            "SPECLINK_SR24_BUCKET_DENSE_ACTIVE_MASK_FUSED":
        ...
        if args.sr24_grouping_trace:
            command_meta["sr24_grouping_trace"] = str(
                run_dir / "speclink_sr24_grouping_trace.jsonl"
            )
        if args.sr24_grouped_queue_shadow:
            command_meta["sr24_grouped_queue_shadow"] = True
            command_meta["sr24_grouped_queue_max_wait_blocks"] = (
                args.sr24_grouped_queue_max_wait_blocks
            )
            command_meta["sr24_grouped_queue_shadow_trace"] = str(
                run_dir / "speclink_sr24_grouping_trace.jsonl"
            )
        if sr24_scheduler_policy_path:
            command_meta["sr24_scheduler_policy_path"] = (
                sr24_scheduler_policy_path
            )
        # SR24 behavior depends on env-gated Python branches that vLLM's
        # default compile-cache key does not currently include. Use a separate
        # cache root per SR24 env fingerprint for both graph-only and default
        # vLLM compile modes so dense/no-op baselines cannot inherit SR24 graphs.
    ...
    elif method in SR24_METHODS:
        min_batch = int(getattr(args, "sr24_min_enabled_batch_size", 0) or 0)
        server_env["SPECLINK_SR24_ENABLE"] = "0"
        command_meta["sr24_effective_enabled"] = False
        command_meta["sr24_batch_fallback"] = True
        command_meta["sr24_batch_fallback_reason"] = (
            f"batch_size={batch_size} < sr24_min_enabled_batch_size={min_batch}"
        )
    else:
        server_env["SPECLINK_SR24_ENABLE"] = "0"
    run_dir.mkdir(parents=True, exist_ok=True)
...
def sr24_static_summary(run_dir: Path,
    ...
    return {
        ...
        "sr24_selective_dense_nonverify_scope_static":
        stats.get("selective_dense_nonverify_scope"),
        "sr24_selective_dense_nonverify_max_rows_static":
        stats.get("selective_dense_nonverify_max_rows"),
        "sr24_selective_dense_nonverify_static_rows_static":
        stats.get("selective_dense_nonverify_static_rows"),
        "sr24_selective_dense_nonverify_layer_ids_by_leaf_static":
        ";".join(
            f"{leaf}={','.join(str(item) for item in layers)}"
            for leaf, layers in sorted(
                (
                    stats.get(
                        "selective_dense_nonverify_layer_ids_by_leaf"
                    ) or {}
        ...
        "sr24_runtime_base_only_layer_ids_by_leaf_static":
        ";".join(
            f"{leaf}={','.join(str(item) for item in layers)}"
            for leaf, layers in sorted(
                (stats.get("runtime_base_only_layer_ids_by_leaf") or {}).items()
            )
        ),
        ...
        "sr24_compile_cache_root_static": (
            ...
            if method in SR24_METHODS else ""
        ),
        "vllm_compile_cache_root_static": (
            str(non_sr24_compile_cache_root(args, method, batch_size))
            if method not in SR24_METHODS else ""
        ),
        "sr24_direct_cslt_linear_static": stats.get("direct_cslt_linear"),
        "sr24_cslt_small_m_alg_id_enabled_static":
        stats.get("cslt_small_m_alg_id_enabled"),
        "sr24_cslt_small_m_threshold_static":
        stats.get("cslt_small_m_threshold"),
        "sr24_cslt_small_m_alg_id_static": stats.get("cslt_small_m_alg_id"),
        "sr24_cslt_small_m_threshold_by_leaf_static":
        json.dumps(
            stats.get("cslt_small_m_threshold_by_leaf") or {},
            sort_keys=True,
        ),
        "sr24_cslt_small_m_alg_id_by_leaf_static":
        json.dumps(stats.get("cslt_small_m_alg_id_by_leaf") or {}, sort_keys=True),
        ...
        "sr24_base_only_dense_verify_layer_ids_static":
        ",".join(
            str(item)
            for item in (stats.get("base_only_dense_verify_layer_ids") or [])
        ),
        "sr24_base_only_dense_verify_layer_ids_by_leaf_static":
        ";".join(
            f"{leaf}={','.join(str(item) for item in layers)}"
            for leaf, layers in sorted(
                (stats.get("base_only_dense_verify_layer_ids_by_leaf") or {}).items()
            )
        ),
        ...
        "sr24_route_overlap_streams":
        stats.get("route_overlap_streams"),
        "sr24_route_overlap_allow_cudagraph":
        stats.get("route_overlap_allow_cudagraph"),
        "sr24_fixed_prefix_route_fastpath":
        stats.get("fixed_prefix_route_fastpath"),
        "sr24_fixed_prefix_route_descriptor_only":
        stats.get("fixed_prefix_route_descriptor_only"),
        "sr24_fixed_block_input_buffer":
        stats.get("fixed_block_input_buffer"),
        "sr24_fixed_block_output_buffer":
        stats.get("fixed_block_output_buffer"),
        "sr24_scheduler_policy_path":
        stats.get("scheduler_policy_path"),
        "sr24_scheduler_policy_allow_legacy_mixed":
        stats.get("scheduler_policy_allow_legacy_mixed"),
        "sr24_scheduler_policy_allow_single_block_packed_parallel":
        stats.get("scheduler_policy_allow_single_block_packed_parallel"),
        "sr24_scheduler_policy_allow_serial_packed_parallel":
        stats.get("scheduler_policy_allow_serial_packed_parallel"),
        "sr24_scheduler_policy_dense_bypass":
        stats.get("scheduler_policy_dense_bypass"),
        "sr24_grouped_queue_shadow":
        stats.get("grouped_queue_shadow"),
        "sr24_grouped_queue_max_wait_blocks":
        stats.get("grouped_queue_max_wait_blocks"),
        "sr24_grouping_trace_enabled":
        stats.get("grouping_trace_enabled"),
        "sr24_grouping_trace_path":
        stats.get("grouping_trace_path"),
        ...
        stats.get("route_min_base_rows_by_leaf"),
        ...
        stats.get("adaptive_dense_fallback_no_residual_only"),
        ...
        stats.get("adaptive_dense_fallback_small_gate_up_no_residual"),
        "sr24_adaptive_dense_fallback_calls":
        ...
        stats.get("bucket_dense_compute_active_only"),
        "sr24_bucket_dense_active_mask_fused":
        stats.get("bucket_dense_active_mask_fused"),
        ...
        ";".join(
            f"{leaf}={','.join(str(item) for item in layer_ids)}"
            for leaf, layer_ids in (
                stats.get("runtime_base_only_layer_ids_by_leaf") or {}
            ).items()
        ),
        "sr24_residual_layer_ids_by_leaf":
        ";".join(
...
def row_for_case(method: str, dataset_name: str, batch_size: int,
    ...
    return {
        ...
        **metrics,
        ...
        "sr24_effective_enabled": (
            sr24_enabled_for_batch(args, method, batch_size)
            if method in SR24_METHODS else ""
        ),
        "sr24_batch_fallback": (
            method in SR24_METHODS
            and not sr24_enabled_for_batch(args, method, batch_size)
        ),
        "sr24_min_enabled_batch_size": (
            int(getattr(args, "sr24_min_enabled_batch_size", 0) or 0)
            if method in SR24_METHODS else ""
        ),
        **(sr24_summary or {}),
...
def build_resume_command(args: argparse.Namespace) -> str:
    parts = [
        ...
        *(["--disable-chunked-prefill"] if args.disable_chunked_prefill else []),
        ...
        str(args.sr24_min_enabled_batch_size),
        "--sr24-threshold",
        ...
        *(
        ...
        str(args.sr24_base_only_dense_verify_layer_ids),
        "--sr24-base-only-dense-verify-layer-ids-by-leaf",
        str(args.sr24_base_only_dense_verify_layer_ids_by_leaf),
        "--sr24-runtime-base-only-layer-ids-by-leaf",
        str(args.sr24_runtime_base_only_layer_ids_by_leaf),
        ...
        *(["--sr24-row-routed-mlp-fixed-block-dense-fill"]
          if args.sr24_row_routed_mlp_fixed_block_dense_fill else []),
        *(["--sr24-fixed-block-input-buffer"]
          if args.sr24_fixed_block_input_buffer else []),
        *(["--sr24-fixed-block-output-buffer"]
          if args.sr24_fixed_block_output_buffer else []),
        ...
        *(
        ...
        str(args.sr24_selective_dense_nonverify_layer_ids_by_leaf),
        ...
        *(["--sr24-fixed-prefix-route-descriptor-only"]
          if args.sr24_fixed_prefix_route_descriptor_only else []),
        *(["--sr24-scheduler-policy-allow-legacy-mixed"]
          if args.sr24_scheduler_policy_allow_legacy_mixed else []),
        *(["--sr24-scheduler-policy-allow-serial-packed-parallel"]
          if args.sr24_scheduler_policy_allow_serial_packed_parallel else []),
        *(["--sr24-scheduler-policy-dense-bypass"]
          if args.sr24_scheduler_policy_dense_bypass else []),
        "--sr24-scheduler-policy-near-full-tolerance",
        str(args.sr24_scheduler_policy_near_full_tolerance),
        *(["--sr24-fixed-block-capacity-padding"]
          if args.sr24_fixed_block_capacity_padding else []),
        *(["--sr24-fixed-block-capacity-zero-dummy"]
          if args.sr24_fixed_block_capacity_zero_dummy else []),
        *(["--sr24-grouping-trace"]
          if args.sr24_grouping_trace else []),
        *(["--sr24-route-overlap-streams"]
          if args.sr24_route_overlap_streams else []),
        *(["--sr24-route-overlap-allow-cudagraph"]
          if args.sr24_route_overlap_allow_cudagraph else []),
        ...
        *(
            ...
            if args.sr24_bucket_dense_compute_active_only
            else []
        ),
        *(
            ["--sr24-bucket-dense-active-mask-fused"]
            if args.sr24_bucket_dense_active_mask_fused
            else []
        ),
        *(
...
def write_outputs(final_root: Path, rows: list[dict[str, Any]],
    ...
    columns = [
        ...
        "sr24_cslt_small_m_alg_id_enabled_static",
        "sr24_cslt_small_m_threshold_static",
        "sr24_cslt_small_m_alg_id_static",
        "sr24_cslt_small_m_threshold_by_leaf_static",
        "sr24_cslt_small_m_alg_id_by_leaf_static",
        "sr24_base_only_dense_nonverify_static",
        "sr24_base_only_allow_compile_static",
...
def apply_sr24_preset(args: argparse.Namespace) -> None:
    ...
    if preset == "gateup_cap0_graph_probe":
        ...
        args.sr24_bucket_dense_copy = True
        # Direct cuSPARSELt is fast for stable decode shapes, but mixed
        # prefill/decode steps can hit cuSPARSELt gather bounds under CUDA
        # Graph replay. Keep pure decode graphable and force mixed steps eager.
    ...
    if preset in {"mlpall_fixedprefix2_directcslt", "mlpall_fixedprefix2_graphsafe"}:
        # Score-free fixed route-table candidates: protect only a fixed prefix2
        # plus the verifier bonus row. directcslt is a pure-decode speed probe;
        # graphsafe keeps CUDA Graph enabled but uses PyTorch's semi-structured
        # Linear wrapper instead of the direct custom cuSPARSELt call.
        args.sr24_backend = "torch_sparse"
        args.sr24_residual_backend = "dense_rows"
        args.sr24_residual_device = "cuda"
        args.sr24_require_gpu_residual = True
    ...
    if preset == "mlpall_tilefill_prefix2_bucket32_cublas":
        # Tile-fill probe: compute a fixed 32-row dense correction bucket with
        # cuBLAS and overwrite sparse-base rows. This keeps the important-token
        # correction shape large enough to use the GPU better than active-only
        # tiny GEMMs, while still letting unimportant rows use the sparse base.
        args.sr24_backend = "torch_sparse"
        args.sr24_residual_backend = "dense_rows"
        args.sr24_residual_device = "cuda"
        args.sr24_require_gpu_residual = True
    ...
    if preset in {
        ...
        # 8pp-budget systems candidate: build fixed route-table rows once per
        # decode step, run important rows through dense MLP, run remaining rows
        # through the 2:4 sparse MLP, and never correct sparse-only rows with a
        # later dense pass. The fixed-prefix descriptor-only route plus
        # reusable input buffers are now part of the preset so direct preset
        # runs do not regress to the older row-list/temporary-allocation path.
        # The min dense row setting fills tiny protected-row sets with extra
        # low-priority rows so the dense branch has usable occupancy. The
        # operator_guard variant follows the packed-MLP planner: do not promote
        # low-priority rows into dense, and fall back to dense unless the
        # sparse branch has about bs64/K8 row fill.
        args.sr24_backend = "torch_sparse"
        args.sr24_residual_backend = "dense_rows"
        args.sr24_residual_device = "cuda"
        args.sr24_require_gpu_residual = True
    ...
    if preset in {
        ...
        # Low-storage lossy candidate.  Older GSM8K/Minerva-50 screens showed
        # this gate_up-only split stayed within the 8pp quality budget while
        # reducing SR24 storage from 1.625x to about 1.25x dense.  Use it to
        # test whether long-output regressions are mainly KV-cache pressure
        # from all-MLP dense_rows residual storage.
        args.sr24_backend = "torch_sparse"
        args.sr24_residual_backend = "dense_rows"
        args.sr24_residual_device = "cuda"
        args.sr24_require_gpu_residual = True
    ...
    if preset == "fixedprefix4_all_rowrouted_graph":
        # Graph-friendly systems probe: protect a fixed prefix plus all
        # non-draft rows, and route dense/sparse rows at the MLP level so rows
        # selected for exact dense execution do not also pay sparse-base work.
        # Unlike fixedprefix4_bucket16_directcslt, this avoids the
        # direct_cpu_route_rows flag that forces eager execution.
        args.sr24_backend = "torch_sparse"
        args.sr24_residual_backend = "dense_rows"
        args.sr24_residual_device = "cuda"
        args.sr24_require_gpu_residual = True
...
SR24_PRESET_OVERRIDE_OPTIONS = {
    ...
    "--sr24-scheduler-policy-dense-bypass",
    "sr24_grouped_queue_shadow":
    "--sr24-grouped-queue-shadow",
    "sr24_grouped_queue_max_wait_blocks":
    "--sr24-grouped-queue-max-wait-blocks",
...
def validate_sr24_runtime_base_only_compile(args: argparse.Namespace) -> None:
    ...
    if not scope:
        return
    if not any(method in SR24_METHODS for method in args.methods):
        return
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "cuSPARSELt. "
            "lossy_prefix2_rowrouted_mlp is the 8pp-budget systems candidate: "
            "fixed-prefix route-table rows, disjoint dense/sparse MLP branches, "
            "descriptor-only route plans, reusable fixed-block input buffers, "
            "and min-dense-row filling to avoid tiny dense GEMMs. "
            "lossy_prefix2_rowrouted_mlp_operator_guard uses the same fixed "
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "If >0, SR24 methods below this client-side batch/concurrency "
            "start the normal dense EAGLE3 server instead of enabling SR24. "
            "The CSV records sr24_batch_fallback=true for these rows. This is "
            "a throughput-guard for current experiments, not a claimed 2:4 "
            "speedup at the fallback batch sizes."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "--sr24-base-only-dense-verify-max-rows. When unset, matching "
            "base_only_24 modules follow the old all-in-scope behavior."
        ),
    )
    parser.add_argument(
        "--sr24-base-only-dense-verify-layer-ids-by-leaf",
        ...
        help=(
            ...
            "'gate_up_proj=31;down_proj=30-31'. This limits which base-only "
            "modules keep a dense copy for small verifier steps."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "--sr24-row-routed-mlp-min-dense-rows is reached. This is an "
            "explicit tile-fill ablation and is off by default."
        ),
    )
    parser.add_argument(
        "--sr24-fixed-block-input-buffer",
        ...
        help=(
            ...
            "data-format/allocator-overhead ablation and is off by default."
        ),
    )
    parser.add_argument(
        "--sr24-fixed-block-output-buffer",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "mask, run the full Llama MLP through the dense weights before trying "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "'gate_up_proj=0-15;down_proj=0-15' to keep only those layers dense, "
            "or 'none' to let no-mask rows use the 2:4 sparse base."
        ),
    )
    parser.add_argument(
        "--sr24-selective-dense-nonverify-max-rows",
        ...
        help=(
            ...
            "base. This avoids low-occupancy sparse kernels on small batches."
        ),
    )
    parser.add_argument(
        "--sr24-selective-residual-policy",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "variable active-row shapes and should be treated as an explicit "
            "operator ablation rather than the graph-safe default."
        ),
    )
    parser.add_argument(
        "--sr24-bucket-dense-active-mask-fused",
        action="store_true",
        help=(
            "When active-only bucket dense correction is enabled, keep the "
            "fixed bucket shape and let the Triton fused GEMM+scatter consume "
            "bucket_values as an active mask. This avoids Python nonzero/"
            "index_select compression and can remain CUDA-Graph compatible, "
            "but it still launches work over the fixed bucket capacity."
        ),
    )
    parser.add_argument(
        "--sr24-sort-bucket-rows",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "plan instead of constructing residual/base row-index tensors. This "
            "is a data-format optimization step toward a grouped/fused mixed "
            "MLP operator and only affects explicitly descriptor-compatible "
            "fixed-prefix plans."
        ),
    )
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional path to a scheduler_policy.json generated by "
            "derive_sr24_packed_mlp_planner.py. When set, the fixed-block "
            "row-routed MLP path uses the policy to decide whether the current "
            "single verifier block may use the mixed dense/sparse operator or "
            "must fall back to dense until grouped verifier blocks are "
            "available. This is an operator-local gate, not a grouped-queue "
            "implementation."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Diagnostic only: let the current live fixed-block dense/sparse "
            "MLP path run even when scheduler_policy.json requests the "
            "microbench-only packed_parallel operator. This measures the "
            "legacy serial fixed-block operator against dense fallback; it "
            "does not implement grouped packed_parallel serving."
        ),
    )
    parser.add_argument(
        "--sr24-scheduler-policy-allow-single-block-packed-parallel",
        action=argparse.BooleanOptionalAction,
        default=True,
        help=(
            "Allow scheduler_policy.json rows with mixed_operator=packed_parallel "
            "to use the current fixed-block overlap path only when the policy "
            "requires a single verifier block. Rows that require grouped "
            "verifier blocks still fall back to dense."
        ),
    )
    parser.add_argument(
        "--sr24-scheduler-policy-allow-serial-packed-parallel",
        action="store_true",
        help=(
            "Allow scheduler_policy.json rows with "
            "mixed_operator=packed_parallel and min_grouped_verifier_blocks<=1 "
            "to use the current serial fixed-block dense/sparse MLP path "
            "without enabling Python stream overlap. This is a live ablation "
            "for testing whether disjoint row routing alone is enough before "
            "writing a fused/grouped packed operator."
        ),
    )
    parser.add_argument(
        "--sr24-scheduler-policy-near-full-tolerance",
        ...
        help=(
            ...
            "policy. For example, tolerance=2 maps active_count=62/63 to a "
            "bs64 policy. Use with --sr24-fixed-block-capacity-padding to "
            "feed a fixed-capacity mixed dense/2:4 operator while discarding "
            "dummy-row outputs."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Experimental fixed-block data-format optimization: when a "
            "near-full scheduler policy maps to a larger batch, pad dense and "
            "2:4 base MLP inputs to the policy capacity with dummy rows, run "
            "the mixed operator at the stable capacity, and assemble only the "
            "real rows. This avoids dense fallback for near-full blocks but "
            "is not a grouped verifier queue."
        ),
    )
    parser.add_argument(
        "--sr24-fixed-block-capacity-zero-dummy",
        action="store_true",
        help=(
            "When capacity padding is enabled, explicitly zero-fill padded "
            "dummy rows before the dense/sparse branch GEMMs. The default "
            "leaves dummy rows undefined because their outputs are discarded "
            "and GEMMs are row-independent; enabling this is a conservative "
            "debug/sanity mode with extra memory-write overhead."
        ),
    )
    parser.add_argument(
        "--sr24-scheduler-policy-dense-bypass",
        action="store_true",
        help=(
            "When scheduler_policy.json says the current fixed-block mixed "
            "operator should fall back to dense, bypass the SR24 MLP hook and "
            "let the original dense vLLM MLP run. This avoids measuring a "
            "slow hook-internal dense fallback as the dense baseline."
        ),
    )
    parser.add_argument(
        "--sr24-grouping-trace",
        action="store_true",
        help=(
            "Write speclink_sr24_grouping_trace.jsonl for each SR24 server run. "
            "The trace records compact fixed-prefix verifier-block counts, "
            "policy minimum grouping requirements, and whether the current "
            "decode step could feed a grouped dense/sparse verifier MLP. It is "
            "diagnostic only and does not change serving behavior."
        ),
    )
    parser.add_argument(
        "--sr24-grouped-queue-shadow",
        action="store_true",
        help=(
            "Enable the default-off SR24 grouped-queue shadow tracer. This "
            "emits sr24_grouped_queue_shadow_decision events into the grouping "
            "trace using the same online bounded-wait policy intended for a "
            "future live scheduler queue. It does not delay verifier execution "
            "or change logits."
        ),
    )
    parser.add_argument(
        ...
        default=0,
        help=(
            "Maximum compatible verifier blocks the SR24 grouped-queue shadow "
            "model may wait before emitting a timeout fallback decision. This "
            "is a trace-only parameter unless --sr24-grouped-queue-shadow is "
            "enabled."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "the output. This is forced out of CUDA Graph mixed replay unless "
            "--sr24-route-overlap-allow-cudagraph is also set, and is meant to "
            "test operator overlap, not as a default serving path."
        ),
    )
    parser.add_argument(
        "--sr24-route-overlap-allow-cudagraph",
        action="store_true",
        help=(
            "Allow fixed-prefix row-routed MLP overlap-stream plans to try CUDA "
            "Graph replay. This is narrower than disabling the SR24 graph guard "
            "globally: dynamic non-fixed-prefix routes still use the normal "
            "force-NONE safety guard."
        ),
    )
    parser.add_argument(
        "--sr24-disable-runtime-stats",
    ...
    if args.sr24_base_only_allow_compile:
        # This flag is used specifically to test/enable the graph-only
        # base_only_24 path. Without sr24_allow_cudagraph, the runner still
        # adds --enforce-eager and the flag silently has no serving effect.
        args.sr24_allow_cudagraph = True
    if args.sr24_breakdown:
        # SR24 breakdown records CUDA events and Python counters protected by
        # locks. Letting vLLM's default Inductor path trace this diagnostic
        # code can fail during startup profiling, so breakdown runs are always
        # eager even if a preset or explicit flag asked for default compile.
        args.sr24_default_vllm_compile = False
        args.sr24_allow_cudagraph = False
        args.sr24_dynamic_auto_cudagraph = False
        args.sr24_force_cudagraph_none_for_mixed = True
