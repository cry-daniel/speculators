...
## 2026-06-29 Current First Read

The current direction should be slowdown-first rather than selector-first. The
best current explicit breakdown is:
...
In that Llama-3.1-8B `math_reasoning`, bs64, EAGLE3 K=8, max-tokens 256
breakdown, normal scheduler/mask build is not the main slowdown
(`0.455 ms/step` clean `speclink_t08`, `0.356 ms/step` in the GPU-count row),
gather/scatter is small (`0.036 ms/event`), and GPU utilization remains high
(`86.938-88.188%` for `speclink_t08`). The slow path is the combination of
CUDA Graph loss and the current two-pass mixed operator:

| component | current evidence | read |
| --- | --- | --- |
| base sparse Linear | `speclink_t08` gate_up16-31 sparse base `0.605 ms/call`; all-corrected `0.741 ms/call` | largest localized GPU-side cost |
| residual correction | `speclink_t08` dense correction `0.333 ms/call`; all-corrected `0.480 ms/call` | large enough to erase sparse-base gains |
| routing rows | `speclink_t08` draft residual/base `8640/4608`, non-draft residual/base `1656/1635`, draft residual fraction `0.652` | quality-safe routing protects too many rows for this operator |
| CUDA Graph | dense `FULL=115,NONE=76,PIECEWISE=1`; base-only `FULL=126,NONE=2`; guarded t08 `NONE=128` | graph coverage is necessary, not sufficient |

## 2026-06-29 Lossy Speed/Quality Check

The latest user constraint permits up to about 8 percentage points of accuracy
loss and asks for a more systems-style path: avoid duplicate sparse+dense work,
pack useful rows, and consider overlap. The first lossy checks show that the
current selector/bucket implementation is still not enough.
...
The b12 critical-prefix bucket candidate has healthy CUDA Graph coverage and
higher accepted draft length, but it only helps at bs64 and still misses the
global goal:

| bs | total speedup | full-batch speedup | accepted draft/step dense -> b12 |
| ---: | ---: | ---: | --- |
| 8 | 0.752x | 0.736x | 1.754 -> 1.944 |
| 16 | 0.776x | 0.790x | 1.809 -> 2.023 |
| 32 | 0.923x | 0.939x | 1.761 -> 2.065 |
| 64 | 1.160x | 1.142x | 1.734 -> 2.209 |

Its GSM8K-50 quality is not acceptable under the new loss budget: dense
EAGLE3 scores `0.7600`, while b12 `speclink_t08` scores `0.5400`, a `-22.0 pp`
drop with `12` paired regressions and `1` paired improvement. So the next work
should not be another smaller global bucket or scalar threshold sweep.

The active-only dense-compute ablation tests the user's suggested execution
shape directly: dense should not recompute inactive rows already handled by
sparse. The current quick implementation is only a diagnostic because it uses
dynamic `nonzero()` compaction. It is CUDA-Graph unsafe, and in eager bs64 it
is slower (`2186.705` full-batch tok/s) than the graph bucket control. It has
been guarded as an eager/diagnostic switch; a real version needs a fixed
device-side row-buffer format or a custom fused kernel.

Updated read: the next promising path is a disjoint row-routed operator, not
all-base-first plus overwrite. Important rows should run dense directly,
unimportant rows should run 2:4 sparse directly, and the output should be
assembled once from graph-stable buffers. If important rows are too few, the
planner should pack across requests/layers or fall back based on measured
thresholds. Two-stream dense/sparse overlap is worth testing only after the row
counts are large enough to avoid tiny-kernel underutilization.

The latest bucket16 Triton dense-GEMM/scatter parameter sweep is not a speed
path for the dominant gate/up correction shape:
...
Best gate/up rows are still `1.41-1.42x` dense and `1.41-1.43x` slower than
the existing bucket-delta path. Down-projection rows are only dense-parity and
still slower than bucket-delta. The next useful optimization is therefore a
fused/grouped sparse-base plus correction operator, or a stronger row signal
that cuts corrected rows without paired accuracy loss.

Follow-up row-routed exact-down microbench roots:
...
These check the closest existing implementation scaffold for avoiding duplicate
sparse-base work: dense gate/up plus dense down for corrected rows, sparse
gate/up plus sparse down for base rows, and final hidden-state assembly. The
best Llama MLP shape row is corrected bucket `128`, with dense MLP graph
`0.8662 ms`, exact-down Triton assemble `0.7488 ms`, and no-final-assemble
`0.7272 ms`. This is only about `1.16x` with final assembly and about `1.19x`
without it, so it still does not provide a robust `1.2x` operator ceiling after
serving overhead. Row-routed exact-down is useful evidence and a scaffold, but
the next speed path still needs a fused/grouped operator or a selector that
keeps corrected rows in this sweet spot without paired accuracy loss.

The rows=1024 follow-up shows why this path is worth keeping as a diagnostic
but also why live serving must be measured before promotion:
...
At bucket `384`, dense MLP graph time is `1.6939 ms` and exact-down Triton
assemble time is `1.3063 ms`, or `0.771x` dense. That is roughly a `1.30x`
operator-only speedup. The subcomponent timings show the cost remains in the
grouped GEMMs themselves, not row assembly: dense-row gate/up plus down is about
`0.616 ms`, base-row sparse gate/up plus down is about `1.971 ms`, and Triton
final assembly is about `0.033 ms`.

The matching bs128 live attempt did not produce an SR24 throughput row:
...
The dense baseline succeeded at bs128 (`3304.055` full-batch tok/s,
`2121.382` total tok/s), but the SR24 server exited during KV-cache allocation.
The log shows SR24 was attached (`storage_over_dense=1.625`) and vLLM then
estimated about `5.44 GiB` of CUDA Graph memory with available KV-cache memory
around `-1.91 GiB`. This is a deployability bottleneck for the all-layer
`dense_rows` row-routed path on this GPU. The right next breakdown is therefore
not another threshold sweep; it is a memory-fitting serving breakdown that
keeps the seven-part table and reports whether the rows=1024 operator headroom
survives after storage, graph, and KV-cache constraints.

## 2026-06-29 bs128 Scoped Quality-Safe Breakdown

The memory-fitting scoped candidate did run at bs128 and gives the current best
answer to the "why slow" question:
...
Setup: Llama-3.1-8B, `math_reasoning`, bs128 client-side concurrency,
EAGLE3 K=8, max new tokens 128, scoped
`gate_up_proj=16-31;down_proj=8-15`, `criticalprefix4_bucket16_directcslt`,
`dense_rows@cuda`, direct cuSPARSELt sparse base, bucket16, graph bucket on.

Clean serving rows:

| method | total tok/s | full-batch tok/s | accepted draft/step | avg GPU util | CUDA Graph | read |
| --- | ---: | ---: | ---: | ---: | --- | --- |
| dense baseline | 2240.098 | 3147.956 | 1.422 | 78.467% | `FULL=8,NONE=55,PIECEWISE=1` | same-root baseline |
| `base_only_24` | 2769.278 | 3871.417 | 1.608 | 91.417% | `FULL=62,NONE=2` | base-only upper bound is real |
| `speclink_t08` | 2681.520 | 3702.182 | 1.633 | 91.500% | `FULL=62,NONE=2` | near but still below robust `1.2x` full-batch |

This answers the first requested point directly: the scoped `base_only_24` is
not slow because of lower accepted length or GPU underutilization. It has
higher accepted draft tokens/step than dense, high GPU utilization, and nearly
all decode steps use full CUDA Graph. Earlier slow base-only rows should be
read as graph/config/scope issues, not as inherent acceptance collapse.

The `speclink_t08` clean row is also graph and utilization healthy, so the
remaining gap to `1.2x` is useful-work efficiency. Diagnostic localization:

| component | bs128 diagnostic value | read |
| --- | --- | --- |
| scheduler / mask build | `6.607 ms/step` in exact diagnostic row, dominated by request-loop accounting | diagnostic synchronization, not clean serving throughput |
| base sparse Linear | `1.004 ms/call` overall; `gate_up_proj=1.054`, `down_proj=0.905` | largest localized GPU-side cost |
| residual correction | `0.163 ms/call` overall; `gate_up_proj=0.181`, `down_proj=0.126` | secondary cost, but enough to keep t08 below base-only |
| gather/scatter | `0.015 ms/call` | not the first bottleneck |
| routing | draft residual/base `4134/3650`, non-draft residual/base `973/1823`, bucket fill `0.964` | bucket capacity is used; many draft rows still require correction |
| CUDA Graph | clean `speclink_t08` `FULL=62,NONE=2` | not the current suspect |
| GPU util | clean `speclink_t08` avg `91.5%`, peak `100%` | not idle GPU |

Exact no-fastpath `all_corrected_24` at the same bs128/max128 scale is clearly
not a speed path:

| method | total tok/s | full-batch tok/s | accepted draft/step | avg GPU util | storage/dense |
| --- | ---: | ---: | ---: | ---: | ---: |
| dense baseline | 2586.544 | 3367.839 | 1.424 | 91.615% | n/a |
| `all_corrected_24` exact no-fastpath | 1994.514 | 2773.572 | 1.392 | 89.438% | 1.625 |

The diagnostic row explains the loss: exact all-corrected pays sparse base
`1.162 ms/call` plus dense residual correction `0.908 ms/call` overall. Per
leaf, gate/up costs `1.360 + 1.088 ms/call`; down costs
`0.766 + 0.548 ms/call`. The clean stats confirm this is GPU-resident:
`sr24_residual_backend_counts={"dense_rows":24}`,
`sr24_residual_device_counts={"cuda:0":24}`, and
`sr24_compressed_residual_runtime_on_gpu=True`. Therefore the problem is not
CPU residual execution; it is the separate sparse-base plus dense-correction
operator design.

Current decision from bs128:

1. `base_only_24` is the valid sparse-base upper bound for this scope.
2. `all_corrected_24` exact no-fastpath should not be optimized by more
   wrapper-level scheduling; it needs a fused/grouped operator or should remain
   a dense-equivalent early-dense control.
3. `speclink_t08` is close enough to justify operator work, but threshold-only
   tuning is unlikely to produce a robust full-batch `1.2x` while preserving
   paired accuracy. The next speed work should target grouped/fused
   sparse-base plus correction, or a stronger row selector that reduces
   corrected rows without creating accepted base-only quality regressions.
...
The optimization target is now explicitly lossy within an 8 percentage-point
accuracy budget, not no-loss paired equivalence. New small-scale serving probes
show that the bottleneck is still operator/data-layout, not just residual-row
selection.

Result roots:

```text
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_lossy_mlpall_prefix5_tritonoverride_speed_b8_16_32_64_20260629/
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_baseonly_upperbound_speed_b8_16_32_64_20260629/
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_route_overlap_fixedprefix_bs64_math128_20260629/
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_baseonly_dense_nonverify_speed_b8_16_32_64_20260629/
```

Llama-3.1-8B, `math_reasoning`, K=8, max new tokens 128:

| candidate | bs8 total | bs16 total | bs32 total | bs64 total | read |
| --- | ---: | ---: | ---: | ---: | --- |
| all-MLP prefix5 + Triton override speedup | 0.490x | 0.516x | 0.659x | 0.712x | negative; too much full-MLP sparse/correction work |
| pure `base_only_24` speedup | 0.622x | 0.589x | 0.735x | 1.185x | useful upper bound only at large batch; small batches underfill sparse kernels |
| pure `base_only_24` full-batch speedup | 0.624x | 0.589x | 0.832x | 1.848x | 2:4 has headroom only when enough verifier rows are packed |
| fixed-prefix routed overlap, bs64 | n/a | n/a | n/a | 0.714x total / 0.786x full-batch | naive side-stream overlap is graph-unsafe and slower |

The fixed-prefix routed overlap probe added
`SPECLINK_SR24_ROUTE_OVERLAP_STREAMS=1`. It confirms the intended split path ran
(`route_all_residual_rows_overlap_stream_calls=1608`), but it also forced CUDA
Graph `NONE` for mixed steps. Breakdown showed average per routed call:

| component | avg ms/call |
| --- | ---: |
| base gather | 0.0069 |
| base sparse GEMM | 0.7440 |
| base index copy | 0.1496 |
| dense gather | 0.0476 |
| dense GEMM | 0.3087 |
| dense index copy | 0.0500 |

This is a useful negative result: launching base sparse and dense correction on
Python-created side streams is not the final path. A publishable system design
should instead use fixed row buffers and graph-capturable streams, or a fused
packed operator that consumes a compact route table and emits the final output
without separate gather, two GEMMs, and two scatters.

`base_only_24 --sr24-base-only-dense-nonverify` is also negative on the current
full-model attachment: it OOMs at attach time because keeping dense non-verify
copies plus full 2:4 storage leaves only ~14 MiB free before a 16 MiB temporary
allocation. Dense fallback must be module/layer scoped, not full-model double
residency.

Current direction:

1. Do not keep sweeping tiny global buckets. Tiny buckets look fast only when
   they leave useful correction rows behind and hurt quality.
2. For bs8/16/32, add shape-aware dense fallback or cross-request packing; the
   current sparse kernels are underfilled even in pure base-only mode.
3. For bs64+, preserve the base-only 2:4 upper bound but avoid duplicate work:
   important-token dense rows must not also pay sparse base, and unimportant
   rows must not pay dense correction.
4. Make the data format explicit: fixed-size route slots for
   `{row_id, route_kind, active}` plus compact per-layer row ranges, so CUDA
   Graph replay sees stable pointers while the GPU skips inactive rows.
5. The next implementation target should be a graph-safe packed mixed MLP:
   sparse base rows and dense important rows either overlap through pre-created
   streams or are handled by one fused Triton/cuSPARSELt-side assembly path.

### 2026-06-29 Scoped Fallback and Quality/Speed Split

The latest code adds a separate dense-verify fallback scope for `base_only_24`:
...
This decouples the sparse attachment scope from the modules that keep a dense
copy for small verifier steps. Without the separate scope, a tail setup such as
`gate_up_proj=16-31;down_proj=8-15` still OOMs at attach because every converted
module also clones dense weights. With the scope narrowed to
`gate_up_proj=31;down_proj=31`, the same bs8 smoke attaches successfully and
records exactly two dense-verify fallback modules.

Result roots:

```text
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_baseonly_denseverify_lastlayer_attachfix_bs8_smoke_20260629/
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_baseonly_denseverify_lastlayer_speed_b64_128_20260629/
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_baseonly_denseverify_lastlayer_gsm8k50_accuracy_20260629/
```

The memory-fitting `base_only_24` path is a good systems upper-bound probe but
not a deployable quality point:

| setup | total speedup | full-batch speedup | GSM8K-50 accuracy |
| --- | ---: | ---: | ---: |
| bs8, last-layer dense verify | 0.759x | 0.767x | not used |
| bs64, last-layer dense verify | 1.141x | 1.178x | not used |
| bs128, last-layer dense verify | 1.397x | 1.810x | not used |
| GSM8K-50 quality | n/a | n/a | dense `0.7400`, base-only `0.1600` |

Read: scoped dense fallback solves the attach memory problem and confirms that
packed 2:4 verifier work can be faster at high batch, but removing residual
correction from most tail modules destroys reasoning accuracy far beyond the
allowed 8 percentage-point budget.

The current quality-safe candidate remains `speclink_t08` with
`criticalprefix4_bucket16_directcslt`:

```text
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_speclink_criticalprefix_gsm8k50_accuracy_20260629/
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_speclink_criticalprefix_speed_b8_16_32_64_20260629/
```

It matched dense on GSM8K-50 (`0.7400/0.7400`, pair reg/imp `0/0`), but the
new max128/math throughput matrix shows it still misses the speed target:

| batch size | total speedup | full-batch speedup | accepted draft/step | avg GPU util |
| ---: | ---: | ---: | ---: | ---: |
| 8 | 0.701x | 0.706x | 1.535 | 89.6% |
| 16 | 0.748x | 0.766x | 1.623 | 89.2% |
| 32 | 0.855x | 0.877x | 1.551 | 88.6% |
| 64 | 0.959x | 1.009x | 1.631 | 90.8% |

This is the clearest quality/speed split so far: the selector can preserve
accuracy, and the sparse upper bound can be fast, but the current
all-base-first plus dense-row correction implementation cannot combine both.

A row-routed MLP smoke tested the right execution idea more directly: important
rows should run dense directly and base rows should run sparse directly, so
important rows do not pay both sparse and dense work.

```text
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_rowrouted_criticalprefix_bs64_speed_smoke_20260629/
```

At bs64/max128 it improves full-batch speed only slightly over the current
critical-prefix path: dense/speclink total `2388.947/2298.897` tok/s
(`0.962x`), full-batch `2764.910/2831.148` tok/s (`1.024x`). It also lowers
runtime memory in the live run compared with broad dense_rows attachment, but
it is not the final 1.2x path.

Next implementation target:

1. Keep `criticalprefix4_bucket16_directcslt` as the current accuracy gate.
2. Use `base_only_24` scoped fallback only as a sparse upper-bound/memory probe.
3. Promote row-routed MLP only after replacing the current Python/Torch dynamic
   row assembly with a graph-stable fixed route-table format or a fused/grouped
   kernel.
4. The publishable systems design should pack route entries as fixed slots,
   fill underfull important rows with near-threshold tokens only when profitable,
   and execute dense-important and sparse-base row groups without duplicate
   work. Pre-created stream overlap is worth revisiting only after pointers and
   buffer shapes are CUDA-Graph stable.

### 2026-06-29 Row-Routed Tile-Fill Follow-Up

The user direction now allows accuracy loss up to 8 percentage points and asks
for a more systems-style path: important rows should run dense directly,
unimportant rows should run 2:4 sparse directly, and underfilled important-row
tiles should be filled only when the operator benefits.
...
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_rowrouted_bucket64_bs64_math128_smoke_20260629/
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_rowrouted_bucket128_bs64_math128_smoke_20260629/
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_rowrouted_bucket64_instrumented_bs64_math64_20260629/
/ACALAB/stu1/chenruiyang/Code/LLM/SpecLink/speculators/examples/evaluate/eval-guidellm/results.bak/sr24_adaptive_bucket64_bs64_math128_smoke_20260629/
```

Clean bs64/math/K8/max128 throughput:

| variant | total speedup | full-batch speedup | avg GPU util | accepted draft/step | read |
| --- | ---: | ---: | ---: | ---: | --- |
| bucket16, min128 | 0.936x | 1.036x | 84.5% | 1.653 | row-routed flags were mostly gated off by the min-dense-row guard |
| bucket64, row-routed | 0.890x | 1.048x | 86.8% | 1.606 | real gate_up/down row routing, but total tok/s worsens |
| bucket128, row-routed | 0.886x | 1.033x | 74.3% | 1.595 | overfills dense rows and underutilizes the run |
| bucket64, adaptive dense fallback | 0.934x | 0.963x | 84.1% | 1.523 | full-layer dense fallback gives up too much sparse benefit |

The refreshed breakdown summary now includes a `Row-Routed Linear Components`
table. For the bucket64 diagnostic row, row routing is real:

| leaf | calls | rows/base/dense per call | dense GEMM ms/call | base sparse ms/call | gather/route/assembly read |
| --- | ---: | ---: | ---: | ---: | --- |
| gate_up | 576 | 285.6 / 221.6 / 64.0 | 0.156 | 0.982 | secondary |
| down | 288 | 285.6 / 221.6 / 64.0 | 0.086 | 0.626 | secondary |

This confirms the current bottleneck after removing duplicate dense/sparse work
from important rows: the remaining base sparse branch is still too expensive
for the routed row shape. Filling important rows from 16 to 64 or 128 does not
create a robust 1.2x path; it mostly trades correction overhead for underfilled
or overfilled GEMMs. The next implementable systems path is therefore a
graph-stable route-table plus fused/grouped mixed operator that keeps large
tiles while avoiding duplicate work. Threshold-only and bucket-only sweeps
should not be considered progress unless they show both speed and an explicit
GSM8K quality gate within the 8pp loss budget.

### 2026-06-29 Lossy Gate and Tiny-Row Routing Check

The current quality target is now `accuracy drop <= 8 percentage points`, so
new candidates should be gated by a bounded-loss script rather than by paired
no-loss. The new runner is:
...
It runs GSM8K `limit=50` first and only launches throughput for candidates that
pass the configured accuracy budget. Outputs stay under `results.bak/`, with
intermediate server work under `temp/`.

Two small bs64/math/K8/max128 probes were run:
...
| candidate | GSM8K-50 dense/SR24 | delta pp | full-batch speedup | total speedup | read |
| --- | ---: | ---: | ---: | ---: | --- |
| `lowresidual_gateup_riskcap2` | `0.7200 / 0.7200` | `0.0` | `1.0185x` | `0.9257x` | quality-safe under 8pp, not fast enough |
| `lowresidual_gateup_riskcap2_rowrouted` | `0.7200 / 0.7200` | `0.0` | `0.9906x` | `0.9192x` | true row-level split, but slower at tiny dense-row counts |

This answers the immediate "important vs unimportant token" question:
important rows should ideally not also pay sparse base, and unimportant rows
should not pay dense. But the current row-routed implementation uses dynamic
`index_select`, a small dense GEMM, a sparse GEMM on the remaining rows, and
`index_copy_` assembly. When the important-token count is only a small bucket,
that path is slower than the base-first overwrite path. Therefore row routing
should keep a min-row/tile-fill guard until a fused or grouped route-table
operator can maintain large, graph-stable tiles.
