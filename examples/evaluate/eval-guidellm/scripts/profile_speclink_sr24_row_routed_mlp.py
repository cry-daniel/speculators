...
def write_report(output_root: Path, rows: list[dict[str, Any]]) -> None:
    ...
    with (output_root / "summary.md").open("w", encoding="utf-8") as handle:
        ...
        handle.write("| bucket | dense gather | base gather | dense gate/up | base gate/up | dense act | base act | dense down | base down | index assemble | Triton assemble |\n")
        handle.write("|-------:|-------------:|------------:|--------------:|-------------:|----------:|---------:|-----------:|----------:|---------------:|----------------:|\n")
        for row in rows:
            handle.write(
                f"| {row['bucket_size']} | "
