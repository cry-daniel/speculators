...
def main() -> None:
    ...
    if torch.cuda.is_available():
        ...
        assert fixed_prefix_all_plan.residual_rows is not None
        assert fixed_prefix_all_plan.base_rows is not None
        assert torch.equal(
        ...
        assert torch.equal(batched_all_mask, expected_all_mask)
        os.environ["SPECLINK_SR24_SELECTIVE_NON_DRAFT_POLICY"] = "bonus"
