...
def _row_has_linear_timing(row: dict[str, Any]) -> bool:
    timing_keys = (
        ...
        "row_routed_down_cuda_ms_per_call",
        "row_routed_down_base_sparse_cuda_ms_per_call",
        "row_routed_down_dense_gemm_cuda_ms_per_call",
...
def _summarize_row(root: Path, row: dict[str, str]) -> dict[str, Any]:
    ...
    row_routed_down_total = _sum_keys(
        cuda_ms,
        [
            "row_routed_down_route_build_cuda_ms",
            "row_routed_down_dense_gather_cuda_ms",
            "row_routed_down_dense_gemm_cuda_ms",
            "row_routed_down_base_gather_cuda_ms",
            "row_routed_down_base_sparse_cuda_ms",
            "row_routed_down_index_copy_cuda_ms",
            "row_routed_down_triton_assemble_cuda_ms",
            "row_routed_down_full_dense_cuda_ms",
        ],
    )
    row_routed_down_base_total = _sum_keys(
        cuda_ms,
        [
            "row_routed_down_base_gather_cuda_ms",
            "row_routed_down_base_sparse_cuda_ms",
    ...
    row_routed_down_dense_total = _sum_keys(
        cuda_ms,
        [
            "row_routed_down_dense_gather_cuda_ms",
            "row_routed_down_dense_gemm_cuda_ms",
    ...
    row_routed_down_gather_scatter_total = _sum_keys(
        cuda_ms,
        [
            "row_routed_down_route_build_cuda_ms",
            "row_routed_down_dense_gather_cuda_ms",
    ...
    result = {
        ...
        "row_routed_down_cuda_ms_total": row_routed_down_total,
        "row_routed_down_cuda_ms_per_call": _ratio(
            row_routed_down_total,
            counts.get("row_routed_down_calls"),
        ),
        "row_routed_down_base_cuda_ms_total": row_routed_down_base_total,
        "row_routed_down_base_cuda_ms_per_call": _ratio(
            row_routed_down_base_total,
            counts.get("row_routed_down_calls"),
        ),
        "row_routed_down_dense_cuda_ms_total": row_routed_down_dense_total,
        "row_routed_down_dense_cuda_ms_per_call": _ratio(
            row_routed_down_dense_total,
            counts.get("row_routed_down_calls"),
        ),
        "row_routed_down_gather_scatter_cuda_ms_total": (
            row_routed_down_gather_scatter_total
        ),
        "row_routed_down_gather_scatter_cuda_ms_per_call": _ratio(
            row_routed_down_gather_scatter_total,
            counts.get("row_routed_down_calls"),
        ),
        "row_routed_down_route_build_cuda_ms_per_call": cuda_avg.get(
            "row_routed_down_route_build_cuda_ms"
        ),
        "row_routed_down_dense_gather_cuda_ms_per_call": cuda_avg.get(
            "row_routed_down_dense_gather_cuda_ms"
        ),
        "row_routed_down_dense_gemm_cuda_ms_per_call": cuda_avg.get(
            "row_routed_down_dense_gemm_cuda_ms"
        ),
        "row_routed_down_base_gather_cuda_ms_per_call": cuda_avg.get(
            "row_routed_down_base_gather_cuda_ms"
        ),
        "row_routed_down_base_sparse_cuda_ms_per_call": cuda_avg.get(
            "row_routed_down_base_sparse_cuda_ms"
        ),
        "row_routed_down_index_copy_cuda_ms_per_call": cuda_avg.get(
            "row_routed_down_index_copy_cuda_ms"
        ),
        "row_routed_down_triton_assemble_cuda_ms_per_call": cuda_avg.get(
            "row_routed_down_triton_assemble_cuda_ms"
        ),
        "row_routed_down_full_dense_cuda_ms_per_call": cuda_avg.get(
            "row_routed_down_full_dense_cuda_ms"
        ),
        "row_routed_down_calls": counts.get("row_routed_down_calls"),
        "row_routed_down_executed": counts.get("row_routed_down_executed"),
        "row_routed_down_full_dense_calls": counts.get(
            "row_routed_down_full_dense_calls"
        ),
        "row_routed_down_cached_plan_hits": counts.get(
            "row_routed_down_cached_plan_hits"
        ),
        "row_routed_down_rows_per_call": _ratio(
            counts.get("row_routed_down_rows"),
            counts.get("row_routed_down_calls"),
        ),
        "row_routed_down_dense_rows_per_call": _ratio(
            counts.get("row_routed_down_dense_rows"),
            counts.get("row_routed_down_calls"),
        ),
        "row_routed_down_base_rows_per_call": _ratio(
            counts.get("row_routed_down_base_rows"),
            counts.get("row_routed_down_calls"),
        ),
        "row_routed_down_skipped_small_dense_rows": counts.get(
            "row_routed_down_skipped_small_dense_rows"
        ),
        "row_routed_down_skipped_large_dense_rows": counts.get(
            "row_routed_down_skipped_large_dense_rows"
        ),
        "row_routed_down_skipped_large_base_rows": counts.get(
            "row_routed_down_skipped_large_base_rows"
        ),
        "row_routed_down_skipped_dense_rows_per_skip": _ratio(
            counts.get("row_routed_down_skipped_dense_rows"),
            _sum_keys(
                counts,
                [
                    "row_routed_down_skipped_small_dense_rows",
        ...
        "row_routed_down_skipped_total_rows_per_skip": _ratio(
            counts.get("row_routed_down_skipped_total_rows"),
            _sum_keys(
                counts,
                [
                    "row_routed_down_skipped_small_dense_rows",
...
def _write_report(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    lines.extend(
        [
            "",
            "## Row-Routed Linear Components",
            "",
            "| source | method | leaf | calls | executed/skipped small | total ms/call | base ms/call | dense ms/call | route | dense gather | dense GEMM | base gather | base sparse | assemble | full dense | rows/base/dense per call | cached plans | skipped dense/total rows |",
            "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        ]
    )
    for idx, row in enumerate(rows, start=1):
        for leaf, prefix in (
            ("gate_up", "row_routed_gate_up"),
            ("down", "row_routed_down"),
        ):
            lines.append(
                "| {idx} | `{method}` | `{leaf}` | {calls} | {executed}/{skipped} | {total} | {base_total} | {dense_total} | {route} | {dense_gather} | {dense_gemm} | {base_gather} | {base_sparse} | {assemble} | {full_dense} | {rows}/{base_rows}/{dense_rows} | {cached} | {skipped_dense}/{skipped_rows} |".format(
                    idx=idx,
                    method=row.get("method", ""),
                    leaf=leaf,
                    calls=_fmt(row.get(f"{prefix}_calls"), 0),
                    executed=_fmt(row.get(f"{prefix}_executed"), 0),
                    skipped=_fmt(row.get(f"{prefix}_skipped_small_dense_rows"), 0),
                    total=_fmt(row.get(f"{prefix}_cuda_ms_per_call")),
                    base_total=_fmt(row.get(f"{prefix}_base_cuda_ms_per_call")),
                    dense_total=_fmt(row.get(f"{prefix}_dense_cuda_ms_per_call")),
                    route=_fmt(row.get(f"{prefix}_route_build_cuda_ms_per_call")),
                    dense_gather=_fmt(
                        row.get(f"{prefix}_dense_gather_cuda_ms_per_call")
                    ),
                    dense_gemm=_fmt(
                        row.get(f"{prefix}_dense_gemm_cuda_ms_per_call")
                    ),
                    base_gather=_fmt(
                        row.get(f"{prefix}_base_gather_cuda_ms_per_call")
                    ),
                    base_sparse=_fmt(
                        row.get(f"{prefix}_base_sparse_cuda_ms_per_call")
                    ),
                    assemble=_fmt(
                        row.get(f"{prefix}_triton_assemble_cuda_ms_per_call")
                        or row.get(f"{prefix}_index_copy_cuda_ms_per_call")
                    ),
                    full_dense=_fmt(
                        row.get(f"{prefix}_full_dense_cuda_ms_per_call")
                    ),
                    rows=_fmt(row.get(f"{prefix}_rows_per_call")),
                    base_rows=_fmt(row.get(f"{prefix}_base_rows_per_call")),
                    dense_rows=_fmt(row.get(f"{prefix}_dense_rows_per_call")),
                    cached=_fmt(row.get(f"{prefix}_cached_plan_hits"), 0),
                    skipped_dense=_fmt(
                        row.get(f"{prefix}_skipped_dense_rows_per_skip")
                    ),
                    skipped_rows=_fmt(
                        row.get(f"{prefix}_skipped_total_rows_per_skip")
                    ),
                )
            )
