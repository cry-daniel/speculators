...
def _keep_mask_is_24(keep: torch.Tensor, *, chunk_rows: int = 512) -> bool:
    ...
    # the full reduction can OOM on large fused Llama projections.
...
def _try_build_critical_prefix_bonus_mask_batched(
    *,
    ...
    def _success_result() -> tuple[
        ...
        }, direct_bucket, direct_route_rows
...
def build_verify_residual_mask(
    *,
    ...
    if static_mask_buffer_enabled():
        ...
        if row_routed_mlp() and static_bucket is not None:
            static_residual_rows = static_bucket[0]
            if row_routed_mlp_reuse_base_output():
    ...
    if row_routed_mlp() and residual_bucket is not None:
        residual_rows = residual_bucket[0]
        if row_routed_mlp_reuse_base_output():
            base_rows = None
            _breakdown_count("scheduler_bucket_base_rows_reuse_base_skips", 1)
...
def _semi_structured_linear(
    ...
    if packed is None:
        return F.linear(input_tensor, weight, bias)
    pad_dense_input = getattr(weight, "_pad_dense_input", None)
    if pad_dense_input is None:
        return F.linear(input_tensor, weight, bias)
    if bias is None:
        return _opaque_cslt_linear(
            ...
            int(getattr(weight, "alg_id_cusparselt", 0)),
        )
    dense_input = pad_dense_input(input_tensor)
    ...
    result = torch._cslt_sparse_mm(
        ...
        alg_id=int(getattr(weight, "alg_id_cusparselt", 0)),
    )
    ...
    if dense_weight is not None and mode() != "base_only":
        ...
        if residual_state is None and _current_residual_mask() is None:
            ...
            if (
                ...
                sr_mode == "selective" and selective_correct_non_draft()
            ):
                if breakdown_linear_enabled():
                    _breakdown_count_module(
    ...
    if reduce_cpu_sync():
        ...
        if dense_weight is not None:
            ...
            if residual_bucket is not None:
                ...
                if linear_breakdown:
                    ...
                    if breakdown_sync_counts():
                        try:
                            ...
                            pass
                if triton_bucket_dense_gemm() and base_output.is_cuda:
                    fused_timer = (
                ...
                dense_input = input_tensor.index_select(0, bucket_rows)
                _breakdown_record_cuda(
                ...
                if bucket_dense_copy():
                    ...
                    if bucket_dense_copy_active_only():
                        ...
                        base_output.index_copy_(0, bucket_rows, dense_output)
                    _breakdown_record_cuda(
...
def row_routed_gate_up_output(
    ...
    if mode() == "base_only" or _current_residual_state() != "mixed":
        return None
    if getattr(module, "_speclink_sr24_profile_leaf", "") != "gate_up_proj":
        return None
    if not sparse_backend_active(module):
        return None
    if getattr(module, "_speclink_sr24_residual_backend", "") != "dense_rows":
    ...
    if dense_count <= 0:
        return None
    if dense_count < row_routed_mlp_min_dense_rows():
        if breakdown_linear_enabled():
        ...
        return None
    max_dense_rows = row_routed_mlp_max_dense_rows()
    ...
    max_base_rows = row_routed_mlp_max_base_rows()
...
def row_routed_down_output(
    ...
    if mode() == "base_only" or _current_residual_state() != "mixed":
        ...
        return None
    if getattr(module, "_speclink_sr24_profile_leaf", "") != "down_proj":
        if linear_breakdown:
...
def row_routed_mlp_output(
    ...
    if breakdown_linear_enabled():
        ...
        _breakdown_count("row_routed_mlp_base_rows", int(base_rows.numel()))
