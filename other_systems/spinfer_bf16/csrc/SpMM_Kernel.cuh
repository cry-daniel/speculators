...
__device__ __forceinline__ __nv_bfloat162 maskloadingv1(uint64_t bitmap, const __nv_bfloat16* __restrict__ startpos, int lane_id) {
    ...
    __nv_bfloat16 A_val1 = (bitmap & bit1) ? startpos[num_ones_before++] : __float2bfloat16(0.0f);
    __nv_bfloat16 A_val2 = (bitmap & bit2) ? startpos[num_ones_before] : __float2bfloat16(0.0f);

    // Combine two __nv_bfloat16 values into a __nv_bfloat162
    return __halves2bfloat162(A_val1, A_val2);
}
__device__ __forceinline__ void SpMM_LoadFragAwithBitmapFromShem(uint32_t __restrict__ a[][4],
                                                         const __nv_bfloat16* __restrict__ ShemVal,
    ...
    if (Pred == true) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                ...
                __nv_bfloat162 val = maskloadingv1(bitmap, ShemVal+start_pos, lane_id);
...
__global__ void SpMM_Kernel_bitmap_v3(const __nv_bfloat16*  A,
...
////////
    ...
    __nv_bfloat16* smem_B = &smem[max_nnz_intile];
...
#pragma unroll(1)
    for (int tile_id_k = 0; tile_id_k < NumIter; tile_id_k++) {
        ...
        // double buffer
        __nv_bfloat16* __restrict__ smem_write_B_PTR = smem_B;
        __nv_bfloat16* __restrict__ smem_read_B_PTR  = smem_B;
...
#pragma unroll
        for (int j = threadIdx.x % WARP_SIZE; j < TilingConfig::TILE_M; j += WARP_SIZE) {  // j-th row
            const int output_index = j + i * M_Global;
            float value = (*(smem_CFrag + i))[j];
            if (Accumulate && Split_K == 1)
                value += __bfloat162float(BlockGlobalPTR[output_index]);
            BlockGlobalPTR[output_index] = __float2bfloat16_rn(value);
        }
