                                                         ...
                                                         int          NNZ_VECTOR_ThisTile2)
{
    ...
    *NNZ_VECTOR_ThreadLocal1 = Num_NNZ_Vector1;
    ...
    constexpr int REGISTERS_PER_TILE =
        TilingConfig::TILE_M == 256
            ? SparseKernelConfig::NUM_REG_FOR_SPARSE_KERNEL / 2
            : SparseKernelConfig::NUM_REG_FOR_SPARSE_KERNEL;
#pragma unroll
    for (int i = 0; i < REGISTERS_PER_TILE / SparseKernelConfig::VECTOR_SIZE; i++) {
...
__device__ __forceinline__ void SpMM_InitSharedMemory(__nv_bfloat16* __restrict__ SharedPTR)
{
    ...
    __nv_bfloat16*     SharedPTR_PerThread = SharedPTR + StartRowNum * TILE_K + HALF_PER_128B * lane_id;
    //
...
#pragma unroll
    for (int i = 0; i < ITERATIONS_PER_THREAD; i++) {
        cp_async_ignore_src<16>(SharedPTR_PerThread, (__nv_bfloat16*)NULL);
        SharedPTR_PerThread += WARP_SIZE * HALF_PER_128B;
    }
}

template<typename TilingConfig, typename SparseKernelConfig>
__device__ __forceinline__ void SpMM_DecompressFromRegisterToShared(__nv_bfloat16* __restrict__ SharedPTR1,
                                                                    uint32_t* Registers_For_SparseTiles1,
                                                                    uint32_t  NNZ_ThreadLocal1,
                                                                    __nv_bfloat16* __restrict__ SharedPTR2,
...
#pragma unroll
            for (int j = 0; j < SparseKernelConfig::VECTOR_SIZE; j++) {
                __nv_bfloat16* half_ptr =
                    reinterpret_cast<__nv_bfloat16*>(&(Registers_For_SparseTiles1[i * SparseKernelConfig::VECTOR_SIZE + j]));
                short* short_ptr  = reinterpret_cast<short*>(half_ptr + 1);
                __nv_bfloat16   value      = *half_ptr;
...
#pragma unroll
                for (int j = 0; j < SparseKernelConfig::VECTOR_SIZE; j++) {
                    __nv_bfloat16* half_ptr =
                        reinterpret_cast<__nv_bfloat16*>(&(Registers_For_SparseTiles2[i * SparseKernelConfig::VECTOR_SIZE + j]));
                    short* short_ptr  = reinterpret_cast<short*>(half_ptr + 1);
                    __nv_bfloat16   value      = *half_ptr;
                    short  index      = *short_ptr;
...
template<typename TilingConfig, typename SparseKernelConfig>
__global__ void SpMM_Kernel(const __nv_bfloat16*  A,
                            ...
                            int          Split_K)
{
    ...
    //
    extern __shared__ __align__(128) __nv_bfloat16 smem[];  // at least be 128 Bytes aligned
...
#pragma unroll(1)
    for (int tile_id_k = 0; tile_id_k < NumIter; tile_id_k++) {
        ...
        // double buffer
        __nv_bfloat16* __restrict__ smem_write_PTR = smem;
        __nv_bfloat16* __restrict__ smem_read_PTR  = smem;
...
// Dense baseline: load-as-dense compute-as-dense
 template<typename TilingConfig, typename SparseKernelConfig>
__global__ void SpMM_Kernel(const __nv_bfloat16 *A, const uint4* Compressed_A, const int *TileOffsets,
                                  const __nv_bfloat16 *B,
                                  __nv_bfloat16* Reduction_Workspace,
                                  ...
                                  )
{
  ...
  const __nv_bfloat16 *ATileGlobalPTR = A + Tile_Start_M * K_Global + BatchID * AverageNumKBlock * TILE_K;
  const __nv_bfloat16 *BTileGlobalPTR = B + Tile_Start_N * K_Global + BatchID * AverageNumKBlock * TILE_K;          // Address
for matrix B, taking SplitK into consideration CopyTileFromGlobalToShared_X_64<TilingConfig::TILE_M, TilingConfig>
  ...
  __syncthreads();
  // Now that shared memory contains all the D tiles, stream them to global memory.
  __nv_bfloat16* BlockGlobalPTR = Reduction_Workspace + BatchID*(M_Global*N_Global) + Tile_Start_M + Tile_Start_N*M_Global;
  #pragma unroll
  for(int i=warpId; i<TilingConfig::TILE_N2; i+=TilingConfig::BLOCK_WARPS)    // i-th column
    #pragma unroll
    for(int j=threadIdx.x%WARP_SIZE; j<TilingConfig::TILE_M; j+=WARP_SIZE) // j-th row
