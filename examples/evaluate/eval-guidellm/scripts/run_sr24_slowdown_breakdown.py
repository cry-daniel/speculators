...
def expand_breakdown_sr24_preset(args: argparse.Namespace) -> None:
    ...
    if source == "mlpall_lowconf_prefix5_tritonoverride":
        ...
        return
    if source == "mlpall_tilefill_prefix2_bucket32_cublas":
        # Keep the breakdown wrapper aligned with the throughput/lm-eval
        # tile-fill preset. Forward a manual expanded config so wrapper defaults
        # cannot accidentally override the child matrix runner's preset.
        args.sr24_backend = "torch_sparse"
        args.sr24_residual_backend = "dense_rows"
        args.sr24_residual_device = "cuda"
        args.sr24_require_gpu_residual = True
    ...
    if source == "criticalprefix_extra2_gateup_scaledbucket":
        ...
        if explicit_residual_bucket_priority:
            args.sr24_residual_bucket_priority = residual_bucket_priority
        return
    if source == "lossy_prefix2_rowrouted_mlp":
        # Keep breakdown runs aligned with the current lossy systems candidate
        # used by the throughput and lm-eval runners: fixed prefix2+bonus rows
        # are dense, all other verifier rows use the 2:4 sparse MLP route, and
        # dense padding keeps tiny protected-row sets from launching very small
        # GEMMs. Preserve an explicit min-base guard such as 256 from the CLI.
        route_min_base_rows = args.sr24_route_min_base_rows
        route_max_dense_fraction = args.sr24_route_max_dense_fraction
        # The breakdown parser's generic default is 128, but this preset's
        # serving/lm-eval definition uses 32. Treat 128 as the generic default
        # unless the caller explicitly passed another value.
        row_routed_min_dense_rows = args.sr24_row_routed_mlp_min_dense_rows
        args.sr24_backend = "torch_sparse"
        args.sr24_residual_backend = "dense_rows"
        args.sr24_residual_device = "cuda"
        args.sr24_require_gpu_residual = True
        ...
        args.sr24_row_routed_mlp_min_dense_rows = (
            row_routed_min_dense_rows
            if row_routed_min_dense_rows not in (0, 128)
            else 32
        )
        args.sr24_route_contiguous_fastpath = True
        args.sr24_route_min_base_rows = (
            route_min_base_rows if route_min_base_rows > 0 else 16
        )
        args.sr24_route_max_dense_fraction = (
            route_max_dense_fraction
            if 0.0 <= route_max_dense_fraction <= 1.0
            else 0.75
        )
        args.sr24_direct_cslt_linear = True
        args.sr24_allow_cudagraph = True
...
def common_matrix_args(
    ...
    *,
    ...
    if args.sr24_selective_dense_nonverify_layer_ids_by_leaf:
        command.extend([
    ...
    command.extend([
        ...
        str(args.sr24_route_min_base_rows_by_leaf),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Forward the selective-mode no-verify dense guard scope to the "
            "matrix runner. Use 'none' for noverify-sparse candidates where "
            "unimportant/no-mask rows should not be recomputed dense."
        ),
    )
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Per-leaf min-base-row fallback thresholds forwarded to the matrix "
            "runner, e.g. 'gate_up_proj=384'."
        ),
    )
    parser.add_argument(
        "--sr24-route-max-dense-fraction",
