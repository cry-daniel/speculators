...
__device__ __forceinline__ void cp_async(__nv_bfloat16* smem_ptr, const __nv_bfloat16* global_ptr, bool pred_guard = true)
{
...
template<int SizeInBytes>
__device__ __forceinline__ void cp_async_test_only(__nv_bfloat16* smem_ptr, const __nv_bfloat16* global_ptr, bool pred_guard = true)
{
    static_assert((SizeInBytes == 4 || SizeInBytes == 8 || SizeInBytes == 16), "Size is not supported");
    unsigned smem_int_ptr = __cvta_generic_to_shared(smem_ptr);
    asm volatile("{ \n"
...
template<int SizeInBytes>
__device__ __forceinline__ void cp_async_ignore_src(__nv_bfloat16* smem_ptr, __nv_bfloat16* global_ptr)
{
    static_assert((SizeInBytes == 4 || SizeInBytes == 8 || SizeInBytes == 16), "Size is not supported");
    unsigned smem_int_ptr = __cvta_generic_to_shared(smem_ptr);
    asm volatile("{ \n"
