...
__device__ __forceinline__ void CopyTileFromGlobalToShared_X_64(__nv_bfloat16* __restrict__ SharedPTR,
...
__device__ __forceinline__ void CopyTileFromGlobalToShared_Sparse(__nv_bfloat16* __restrict__ SharedPTR,
                                                               const __nv_bfloat16* GlobalPTR,
                                                               const int   NNZ,
                                                               bool        Pred = true)
{
    ...
    for(int i = threadIdx.x; i < NNZ_8; i+= threadPerBlock) {
        const __nv_bfloat16* GlobalPTR_Unit        =  GlobalPTR + i * 8;
        __nv_bfloat16* __restrict__ SharedPTR_Unit = SharedPTR + i * 8;
...
__device__ __forceinline__ __nv_bfloat162 maskloadingv2(uint64_t bitmap, const __nv_bfloat16* __restrict__ startpos, int lane_id) {
                                                         ...
                                                         int bit_offset)
{
    ...
    for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                ...
                __nv_bfloat162 val = maskloadingv2(bitmap, ShemVal+*start_pos, lane_id);
...
__device__ __forceinline__ void PipelinedCoreComputationsBitmapV2(float c[][REG_PER_C_TENSOR_16_16],
                                                          uint32_t __restrict__ a[][4],
                                                          uint32_t __restrict__ b[][4],
                                                          __nv_bfloat16* __restrict__ ShemAVal,
