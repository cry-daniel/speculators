...
static void SpMM_SplitK_Kernel_Ex_bitmap_v3(cudaStream_t stream,
                                  ...
                                  const __nv_bfloat16* Compressed_A,
                                  ...
                                  bool         Accumulate)
{
    // Upstream sized the compressed-value staging area for one 13B pruning
    // point (2304 values).  An exact 3:4 tile can contain 3072 values, and the
    // public N:M adapter accepts arbitrary densities.  Reserve the correctness
    // upper bound of one full 64x64 tile; the kernel still starts activation
    // staging at the runtime max_nnz value, so unused bytes are not read.
    static int SHMEM_SZ = max((TilingConfig::TILE_N * TILE_K) * sizeof(__nv_bfloat16) * 2 + 4096 * sizeof(__nv_bfloat16) + (TilingConfig::TILE_BITMAP_M_V3 * TilingConfig::TILE_BITMAP_K_V3) * sizeof(uint64_t),
    ...
    SpMM_Kernel_bitmap_v3<TilingConfig><<<GridDim, BlockDim, SHMEM_SZ, stream>>>(
        A, Compressed_A, TileOffsets, TileOffsets_Median, bitmap, max_nnz_intile, B, Reduction_Workspace, M_Global, N_Global, K_Global, Split_K, Accumulate);
}
...
cudaError_t SpMM_SplitK_API_bitmap_v3(cudaStream_t stream,
                            ...
                            const int    N_Global,
                            const int    K_Global,
                            __nv_bfloat16*        Reduction_Workspace,  // Identical workspace for all SpMM kernel launchesSpMM_SplitK_Kernel_Ex_bitmap
                            int          Split_K,
                            bool         Accumulate)
{
    __nv_bfloat16* SpMM_SplitK_OutputPTR;
    if (Split_K == 1)
    ...
    switch (N_Global) {
        case 8:
            SpMM_SplitK_Kernel_Ex_bitmap_v3<TilingConfigBitmapV3<4, 1, 1, 1>>(
                stream, A, Compressed_A, TileOffsets, TileOffsets_Median, bitmap, max_nnz_intile, B, SpMM_SplitK_OutputPTR, M_Global, N_Global, K_Global, Split_K, Accumulate);
            break;
        case 16:
    ...
    SplitK_Reduction<<<GridDim, BlockDim, 0, stream>>>(C, Reduction_Workspace, M_Global, N_Global, Split_K, Accumulate);
    ...
    int& max_nnz_count)
{
    ...
    *Compressed_Val = (__nv_bfloat16*)malloc(M * K * sizeof(__nv_bfloat16));
    ...
    for (int global_tile_m = 0; global_tile_m < num_global_tiles_M; ++global_tile_m) {
        for (int global_tile_k = 0; global_tile_k < num_global_tiles_K; ++global_tile_k) {
            ...
            for (int p = 0; p < global_padding; ++p) {
                (*Compressed_Val)[val_count++] = __float2bfloat16(0.0f);
            }
    ...
    *Compressed_Val = (__nv_bfloat16*)realloc(*Compressed_Val, val_count * sizeof(__nv_bfloat16));
                                       ...
                                       char* OutputSizesFileName)
{
    ...
    __nv_bfloat16* Compressed_Val_cpu_v3 = nullptr;
    ...
    out_SizesFile.close();
    out_CompressedvalFile.write((char*)Compressed_Val_cpu_v3, sizeof(__nv_bfloat16) * val_count_v3);
