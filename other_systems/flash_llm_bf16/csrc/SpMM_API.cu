...
static void SpMM_SplitK_Kernel_Ex(cudaStream_t stream,
                                  ...
                                  int          Split_K)
{
    static int SHMEM_SZ = max((TilingConfig::TILE_M * TILE_K + TilingConfig::TILE_N * TILE_K) * sizeof(__nv_bfloat16) * 2,
...
/*
__nv_bfloat16* Reduction_Workspace:  1. Requiring an extra memory space in device memory for un-reducted intermediate output
tensors
...
cudaError_t SpMM_SplitK_API(cudaStream_t stream,
                            ...
                            const int    N_Global,
                            const int    K_Global,
                            __nv_bfloat16*        Reduction_Workspace,  // Identical workspace for all SpMM kernel launches
                            int          Split_K)
{
#ifdef DEBUG_MODE
    printf(
    ...
    switch (N_Global) {
        case 8:
            SpMM_SplitK_Kernel_Ex<TilingConfig<4, 1, 1, 1>, SparseKernelConfig<128>>(
                stream, A, Compressed_A, TileOffsets, B, SpMM_SplitK_OutputPTR, M_Global, N_Global, K_Global, Split_K);
            break;
        case 16:
            SpMM_SplitK_Kernel_Ex<TilingConfig<4, 1, 1>, SparseKernelConfig<128>>(
                stream, A, Compressed_A, TileOffsets, B, SpMM_SplitK_OutputPTR, M_Global, N_Global, K_Global, Split_K);
            break;
        case 32:
            SpMM_SplitK_Kernel_Ex<TilingConfig<4, 1, 2>, SparseKernelConfig<128>>(
                stream, A, Compressed_A, TileOffsets, B, SpMM_SplitK_OutputPTR, M_Global, N_Global, K_Global, Split_K);
            break;
        case 64:
...
*/
// template<typename TilingConfig, typename SparseKernelConfig>
__host__ int InitSparseMatrixA_API(__nv_bfloat16*      A_h,
                                   int        M,
                                   int        N,
                                   int        K,
                                   ...
                                   int**      TileOffsets)        // CPU_PTR
{
    ...
    for (int i = 0; i < M; i++)
        for (int j = 0; j < K; j++)
            if (fabs(__bfloat162float(A_h[i * K + j])) > ZERO_THRESHOLD)
                NNZ_Original++;
#ifdef DEBUG_MODE
    ...
    for (int i = 0; i < M / TILE_M; i++) {
        for (int j = 0; j < K / TILE_K; j++) {
            __nv_bfloat16* CurrentTilePTR = A_h + (i * TILE_M) * K + (j * TILE_K);
            int   TileNZCount    = 0;
            for (int m = 0; m < TILE_M; m++) {
                for (int n = 0; n < TILE_K; n++) {
                    __nv_bfloat16 value = CurrentTilePTR[m * K + n];
                    if (fabs(__bfloat162float(value)) > ZERO_THRESHOLD)
                        TileNZCount++;
                }
            }
        ...
        for (int j = 0; j < K / TILE_K; j++) {
            ...
            for (int m = 0; m < TILE_M; m++) {
                for (int n = 0; n < TILE_K; n++) {
                    ...
                    if (fabs(__bfloat162float(value)) > ZERO_THRESHOLD) {
                        __nv_bfloat16* half_ptr   = reinterpret_cast<__nv_bfloat16*>(&Item);
                        *half_ptr        = value;
            ...
            for (int k = 0; k < remainingPaddings; k++) {
                ...
                //
                __nv_bfloat16* half_ptr   = reinterpret_cast<__nv_bfloat16*>(&Item);
                *half_ptr        = __float2bfloat16_rn(0.0f);
                short* short_ptr = reinterpret_cast<short*>(half_ptr + 1);
...
__host__ int InitSparseMatrixA_API_NoReorder(__nv_bfloat16*      A_h,
                                             int        M,
                                             int        N,
                                             int        K,
                                             uint32_t** Compressed_A,  // CPU PTR
        ...
        for (int j = 0; j < K / TILE_K; j++) {
            ...
            for (int m = 0; m < TILE_M; m++) {
                for (int n = 0; n < TILE_K; n++) {
                    __nv_bfloat16 value = CurrentTilePTR[m * K + n];
                    if (fabs(__bfloat162float(value)) > ZERO_THRESHOLD) {
                        __nv_bfloat16* half_ptr   = reinterpret_cast<__nv_bfloat16*>(*Compressed_A + TotalNZCount);
                        *half_ptr        = value;
                        short* short_ptr = reinterpret_cast<short*>(half_ptr + 1);
                        // Row permutation for bank-conflict-free shared memory layout
                        int      row            = m;
